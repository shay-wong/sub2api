export default {
    settings: {
      ...
      security: {
        ...
        sessionBindingHint: 'Bind sessions to the trusted client IP and User-Agent. Configure trusted proxies first; this is disabled by default because a mismatch immediately revokes the session.',
      ...
      panelRateLimit: {
        ...
        loadFailed: 'Failed to load panel rate limit settings. Retry before making changes.',
        retry: 'Retry',
      ...
      apiKeyAcl: {
        ...
        description: 'Choose which client IP is used by API Key IP allowlists and denylists',
        trustForwardedIp: 'Trust forwarded client IP',
        trustForwardedIpHint:
          'Enabled by default for upgrade compatibility. When enabled, raw CF-Connecting-IP, X-Real-IP, or X-Forwarded-For values take over server.trusted_proxies for API Key access control and request metadata. Disable it to enforce the Gin trusted-proxy chain configured by server.trusted_proxies. Only enable takeover mode when the origin cannot be reached directly. Session binding always uses the trusted-proxy chain.',
      ...
      openaiFastPolicy: {
        title: 'OpenAI Service Tier Policy',
        description: 'Intercept, filter, or pass OpenAI ultrafast / fast(priority) / flex requests based on the request body service_tier field. Applies to the OpenAI gateway only.',
