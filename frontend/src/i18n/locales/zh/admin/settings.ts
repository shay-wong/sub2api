    ...
    settings: {
      ...
      apiKeyAcl: {
        ...
        trustForwardedIpHint:
          '为保证升级兼容默认开启。开启后 CF-Connecting-IP、X-Real-IP 或 X-Forwarded-For 会接管 API Key 访问控制与请求元数据的客户端 IP 解析；关闭后严格使用 server.trusted_proxies 配置的 Gin 可信代理链。仅在源站无法被直接访问时开启接管模式。会话绑定始终使用可信代理链。',
