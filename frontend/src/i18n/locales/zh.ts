export default {
  ...
  admin: {
    ...
    projects: {
      ...
      appProfiles: '应用配置',
      profilesDescription: '项目级可选择不限制，或选择一份应用配置限制可见的分组、账号和订阅。',
      addProfile: '新增配置',
      editProfile: '编辑配置',
      profileName: '配置名称',
      profileMode: '项目资源范围',
      initialBindings: '初始资源配置',
      initialBindingsHint: '创建项目时直接选择默认配置包含的分组、账号和订阅；切换不限制不会清空这份配置。',
      resourceBindings: '资源绑定',
      resourceBindingsHint: '配置内只维护分组、账号和订阅；用户和 API Key 在成员里管理。',
      resourceBindingsLoadFailedHint: '资源绑定加载失败，本次只会保存配置名称和描述，不会修改已绑定资源。',
      createRestrictedHint: '只显示并管理当前配置绑定的资源。',
      createUnrestrictedHint: '项目级不限制资源，但默认配置仍会保留所选分组、账号和订阅，后续可切回使用。',
      activeScope: '生效范围',
      activeProfile: '生效中',
      ...
      unrestrictedHint: '项目级不限制资源，仪表盘、运维监控和管理列表会按当前项目上下文查看全部资源。',
      ...
      noProjects: '暂无项目空间',
      noProjectsDescription: '创建项目后，可以配置这个空间的成员，以及可见的分组、账号和订阅。',
      noMembers: '暂无成员',
