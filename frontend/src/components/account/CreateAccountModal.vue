  ...
  >
    ...
    <div v-else class="space-y-5">
      <OAuthAuthorizationFlow
        ...
        :show-codex-session-import-option="form.platform === 'openai' && authStore.isAdmin"
        :show-agent-identity-option="form.platform === 'openai' && authStore.isAdmin"
...
watch(apiKeyBaseUrl, (baseURL) => {
  if (form.platform === 'openai' && !openAIEndpointCapabilitiesTouched.value) {
    openAIEndpointCapabilities.value = inferOpenAIEndpointCapabilities(baseURL)
  }
})
...
const handleGenerateUrl = async () => {
  const proxyId = authStore.isAdmin ? form.proxy_id : null
  if (form.platform === 'openai') {
    ...
    await oauth.generateAuthUrl(addMethod.value, proxyId)
  }
}
...
const createAccountAndFinish = async (
  ...
  await doCreateAccount({
    ...
    credentials,
    extra: finalExtra,
    proxy_id: authStore.isAdmin ? form.proxy_id : null,
...
const handleOpenAIExchange = async (authCode: string) => {
  ...
  try {
    ...
    const tokenInfo = await oauthClient.exchangeAuthCode(
      ...
      stateToUse,
      authStore.isAdmin ? form.proxy_id : null
    )
    if (!tokenInfo) return
...
const handleGeminiExchange = async (authCode: string) => {
  ...
  try {
    ...
    const tokenInfo = await geminiOAuth.exchangeAuthCode({
      ...
      state: stateToUse,
      proxyId: authStore.isAdmin ? form.proxy_id : null,
...
const handleAnthropicExchange = async (authCode: string) => {
  ...
  try {
    const proxyConfig = authStore.isAdmin && form.proxy_id ? { proxy_id: form.proxy_id } : {}
    const endpoint =
      addMethod.value === 'oauth'
