...
const startTest = async () => {
  ...
  if (!account || !canStartTest.value) return
  ...
  try {
    await runAccountConnectionTest({
      ...
      modelId: showModelSelect.value ? selectedModelId.value : undefined,
      prompt: supportsPromptInput.value ? testPrompt.value.trim() : isOpenAIAccount.value || isGrokAccount.value ? '' : undefined,
      mode: isGrokAccount.value ? grokTestMode.value : isOpenAIAccount.value ? testMode.value : undefined,
      imageDataURL: supportsImageUpload.value ? uploadImageDataURL.value || undefined : undefined,
      audioDataURL: supportsAudioUpload.value ? uploadAudioDataURL.value || undefined : undefined,
