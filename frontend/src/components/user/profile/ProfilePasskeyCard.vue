<template>
  <div class="card">
      ...
      <div>
        ...
        >
          <div class="grid gap-3 sm:grid-cols-2">
            ...
            <div v-else>
              <label for="passkey-add-email-code" class="input-label">{{
                t('profile.passkey.emailCode')
              }}</label>
              <div class="flex gap-2">
                <input
                  id="passkey-add-email-code"
                  v-model="newEmailCode"
                  class="input min-w-0 flex-1"
                  inputmode="numeric"
                  autocomplete="one-time-code"
                  maxlength="6"
                  :placeholder="t('profile.passkey.emailCodePlaceholder')"
                />
                <button
                  type="button"
                ...
                >
                  {{ sendingCode ? t('common.sending') : t('profile.passkey.sendCode') }}
                </button>
              </div>
            </div>
          </div>
          <div class="flex justify-end gap-2">
            ...
            <button type="submit" class="btn btn-primary" :disabled="busy || !canAdd">
    ...
    <!-- 删除确认：吊销凭据需二次验证，防止被窃会话静默移除 Passkey -->
      ...
      <div class="flex min-h-full items-center justify-center p-4">
        ...
        >
          ...
          <form class="mt-4 space-y-4" @submit.prevent="confirmDelete">
            ...
            <div v-else>
              <label for="passkey-delete-email-code" class="input-label">{{
                t('profile.passkey.emailCode')
              }}</label>
              <div class="flex gap-2">
                <input
                  id="passkey-delete-email-code"
...
const verificationMethod = ref<PasskeyVerificationMethod>('password')
const newName = ref('')
...
const canDelete = computed(() =>
  verificationMethod.value === 'email'
    ? deleteEmailCode.value.length === 6
    : deletePassword.value.length > 0
)

function verification(emailCode: string, password: string): PasskeyVerification {
  return verificationMethod.value === 'email' ? { email_code: emailCode } : { password }
}
...
async function loadCredentials(): Promise<void> {
  ...
  loading.value = true
  try {
    const [loadedCredentials, loadedMethod] = await Promise.all([
      passkeyAPI.list(),
      passkeyAPI.getVerificationMethod()
    ])
    credentials.value = loadedCredentials
    verificationMethod.value = loadedMethod
  } catch (error) {
...
async function addPasskey(): Promise<void> {
  ...
  try {
    await passkeyAPI.register(
      newName.value.trim(),
      verification(newEmailCode.value, newPassword.value)
    )
...
async function confirmDelete(): Promise<void> {
  ...
  if (!credential || !canDelete.value) return
  busy.value = true
  try {
    await passkeyAPI.remove(
      credential.id,
      verification(deleteEmailCode.value, deletePassword.value)
    )
...
async function sendVerifyCode(): Promise<void> {
  ...
  try {
    await passkeyAPI.sendVerifyCode()
    appStore.showSuccess(t('profile.passkey.codeSent'))
  } catch (error) {
    appStore.showError(extractErrorMessage(error, t('profile.passkey.sendCodeFailed')))
  } finally {
