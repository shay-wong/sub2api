...
describe('passkey api', () => {
  ...
  it('sends the account password when beginning registration', async () => {
    ...
    await passkeyAPI.register('Laptop', { password: 'hunter2' })
  ...
  it('sends the account password when revoking a credential', async () => {
    ...
    await passkeyAPI.remove(12, { password: 'hunter2' })
  ...
  it('uses backend-selected email verification for passkey changes', async () => {
    get.mockResolvedValueOnce({ data: { method: 'email' } })
    post.mockResolvedValueOnce({ data: { success: true } })
    remove.mockResolvedValueOnce({ data: null })

    await expect(passkeyAPI.getVerificationMethod()).resolves.toBe('email')
    await passkeyAPI.sendVerifyCode()
    await passkeyAPI.remove(12, { email_code: '123456' })

    expect(get).toHaveBeenCalledWith('/user/passkeys/verification-method')
    expect(post).toHaveBeenCalledWith('/user/passkeys/send-code')
    expect(remove).toHaveBeenCalledWith('/user/passkeys/12', {
      data: { email_code: '123456' }
    })
  })
})
