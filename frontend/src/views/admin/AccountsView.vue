<template>
  <AppLayout>
    <TablePageLayout>
      ...
      <template #table>
        <AccountBulkActionsBar
          ...
          :can-test="allSelectedAccountsLoaded"
          :can-probe-upstream-billing="isFullAdmin"
    ...
    <AccountActionMenu :show="menu.show" :account="menu.acc" :position="menu.pos" :show-schedule="isFullAdmin" @close="menu.show = false" @test="handleTest" @stats="handleViewStats" @schedule="handleSchedule" @duplicate="handleDuplicateAccount" @reauth="handleReAuth" @refresh-token="handleRefresh" @recover-state="handleRecoverState" @reset-quota="handleResetQuota" @set-privacy="handleSetPrivacy" @create-spark-shadow="handleCreateSparkShadow" />
...
import type { Account, AccountPlatform, AccountSchedulerGroupScore, AccountType, ProxyOption as AccountProxy, AdminGroup, WindowStats, ClaudeModel, AdminAccountDataFormat, UpstreamBillingProbeSettings, UpstreamBillingProbeSnapshot } from '@/types'
...
const saveUpstreamBillingProbeSettings = async () => {
  ...
  try {
    ...
    upstreamBillingProbeGloballyEnabled.value = saved.enabled
...
const allSelectedAccountsLoaded = computed(() => selectedAccountsForBulkDelete.value.length === selIds.value.length)
...
const confirmBulkDelete = async () => {
  ...
  try {
    ...
    const failedIds = result.failed_ids?.length ? result.failed_ids : (result.failed > 0 ? accountIds : [])
    const deletedIds = result.success_ids?.length
      ? result.success_ids
      : accountIds.filter(id => !failedIds.includes(id))
...
const handleDataImported = (options?: { close?: boolean }) => {
