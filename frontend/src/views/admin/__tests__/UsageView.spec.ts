...
const { list, exportList, count, getStats, getSnapshotV2, getById, getModelStats, searchModels, listErrorLogs, authState, routeQuery, aoaToSheet, sheetAddAoa, saveAs, xlsxWrite } = await vi.hoisted(async () => {
...
const UsageFiltersStub = defineComponent({
  ...
  template: '<div data-test="usage-filters" :data-show-cleanup="String(showCleanup)"><span data-test="user-filter-label">{{ userKeyword }}</span><slot name="after-reset" /></div>',
})
const UsageTableStub = {
  props: ['allowUserDetail', 'columns'],
  emits: ['userClick'],
...
const mountRouteFilteredUsageView = () => mount(UsageView, {
  global: { stubs: {
    ...
    UserBalanceHistoryModal: true, AuditLogModal: true, OpsErrorLogTable: OpsErrorLogTableStub,
    OpsErrorDetailModal: OpsErrorDetailModalStub, Pagination: true, Select: true,
...
describe('admin UsageView route filters', () => {
  beforeEach(() => {
    ...
    count.mockReset().mockResolvedValue({ total: 0 })
    ...
    getById.mockReset()
    searchModels.mockReset().mockResolvedValue([])
...
describe('admin UsageView model audit export', () => {
	beforeEach(() => {
		vi.useFakeTimers()
		list.mockReset().mockResolvedValue({ items: [], total: 0, pages: 0 })
		count.mockReset().mockResolvedValue({ total: 1 })
