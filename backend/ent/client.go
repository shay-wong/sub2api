...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		...
		c.PaymentProviderInstance, c.PendingAuthSession, c.Project, c.ProjectMember,
		c.ProjectProfile, c.ProjectProfileBinding, c.PromoCode, c.PromoCodeUsage,
...
// QueryProject queries the project edge of a Account.
func (c *AccountClient) QueryProject(_m *Account) *ProjectQuery {
	query := (&ProjectClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
// QueryProject queries the project edge of a Group.
func (c *GroupClient) QueryProject(_m *Group) *ProjectQuery {
	query := (&ProjectClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
// QueryGroups queries the groups edge of a Project.
func (c *ProjectClient) QueryGroups(_m *Project) *GroupQuery {
	query := (&GroupClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		...
		PaymentProviderInstance, PendingAuthSession, Project, ProjectMember,
		ProjectProfile, ProjectProfileBinding, PromoCode, PromoCodeUsage, Proxy,
		RedeemCode, SecuritySecret, Setting, SubscriptionPlan, TLSFingerprintProfile,
		UsageCleanupTask, UsageLog, User, UserAllowedGroup, UserAttributeDefinition,
		UserAttributeValue, UserGroupRateLimitWindow, UserPlatformQuota,
