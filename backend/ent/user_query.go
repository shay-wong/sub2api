...
type UserQuery struct {
	...
	withGroupRateLimitWindows *UserGroupRateLimitWindowQuery
...
func (_q *UserQuery) QueryProjectMembers() *ProjectMemberQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, user.ProjectMembersTable, user.ProjectMembersColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
// QueryGroupRateLimitWindows chains the current query on the "group_rate_limit_windows" edge.
func (_q *UserQuery) QueryGroupRateLimitWindows() *UserGroupRateLimitWindowQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withGroupRateLimitWindows: _q.withGroupRateLimitWindows.Clone(),
...
// WithGroupRateLimitWindows tells the query-builder to eager-load the nodes that are connected to
// the "group_rate_limit_windows" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithGroupRateLimitWindows(opts ...func(*UserGroupRateLimitWindowQuery)) *UserQuery {
	query := (&UserGroupRateLimitWindowClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withGroupRateLimitWindows = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [16]bool{
			...
			_q.withGroupRateLimitWindows != nil,
	...
	if query := _q.withGroupRateLimitWindows; query != nil {
		if err := _q.loadGroupRateLimitWindows(ctx, query, nodes,
			func(n *User) { n.Edges.GroupRateLimitWindows = []*UserGroupRateLimitWindow{} },
			func(n *User, e *UserGroupRateLimitWindow) {
				n.Edges.GroupRateLimitWindows = append(n.Edges.GroupRateLimitWindows, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadGroupRateLimitWindows(ctx context.Context, query *UserGroupRateLimitWindowQuery, nodes []*User, init func(*User), assign func(*User, *UserGroupRateLimitWindow)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(usergroupratelimitwindow.FieldUserID)
	}
	query.Where(predicate.UserGroupRateLimitWindow(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.GroupRateLimitWindowsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
