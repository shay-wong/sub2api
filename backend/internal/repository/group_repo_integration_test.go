...
func (s *GroupRepoSuite) TestCreateFromSourcePreservesPriorityAndFiltersIneligibleAccounts() {
	...
	insertAccount := func(name, accountType string, deleted bool) int64 {
		...
		s.Require().NoError(scanSingleRow(
			s.ctx,
			s.tx,
			"INSERT INTO accounts (project_id, name, platform, type, deleted_at) VALUES ($1, $2, $3, $4, $5) RETURNING id",
			[]any{projectID, name, service.PlatformOpenAI, accountType, deletedAt},
			&id,
		))
		return id
	}
...
func (s *GroupRepoSuite) TestCreateFromSourceIgnoresLegacyProjectProfileBindings() {
	...
	source := mustCreateGroup(s.T(), s.tx.Client(), &service.Group{
		...
		Name:      "restricted-duplicate-source",
		Platform:  service.PlatformAnthropic,
		Status:    service.StatusActive,
	})
	visibleAccountID := s.insertAccountInProject(homeProjectID, "restricted-duplicate-visible")
	hiddenAccountID := s.insertAccountInProject(homeProjectID, "restricted-duplicate-hidden")
	for _, accountID := range []int64{visibleAccountID, hiddenAccountID} {
		_, err := s.tx.ExecContext(
			s.ctx,
	...
	duplicate := &service.Group{
		...
		Name:             "restricted-duplicate-copy",
		Platform:         source.Platform,
		RateMultiplier:   1,
		Status:           "inactive",
		SubscriptionType: service.SubscriptionTypeStandard,
	...
	projectCtx := s.ctx
	s.Require().NoError(s.repo.CreateFromSource(projectCtx, duplicate, source.ID))
	...
	s.Require().NoError(err)
	defer func() { _ = rows.Close() }()
	copiedAccountIDs := make([]int64, 0, 2)
	for rows.Next() {
		var copiedAccountID int64
		s.Require().NoError(rows.Scan(&copiedAccountID))
		copiedAccountIDs = append(copiedAccountIDs, copiedAccountID)
	}
	s.Require().ElementsMatch([]int64{visibleAccountID, hiddenAccountID}, copiedAccountIDs)
}
...
func (s *GroupRepoSuite) TestUpdateSortOrdersIgnoresLegacyProjectProfileBindings() {
	...
	s.Require().NoError(err)
	got, err = s.repo.GetByID(projectCtx, unboundGroup.ID)
	s.Require().NoError(err)
	s.Require().Equal(24, got.SortOrder)
}
...
func (s *GroupRepoSuite) TestDeleteAccountGroupsByGroupIDIgnoresLegacyProjectProfileBindings() {
	...
	affected, err := s.repo.DeleteAccountGroupsByGroupID(s.ctx, group.ID)
	s.Require().NoError(err)
