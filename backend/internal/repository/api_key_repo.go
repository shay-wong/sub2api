...
func (r *apiKeyRepository) Create(ctx context.Context, key *service.APIKey) error {
	projectID, err := resolveProjectIDForCreate(ctx, r.sql, key.ProjectID)
	if err != nil {
		return err
	}
...
func (r *apiKeyRepository) Update(ctx context.Context, key *service.APIKey, fields service.APIKeyUpdateFields) error {
	...
	if fields.RateLimit5h {
		builder.SetRateLimit5h(key.RateLimit5h)
	}
	if fields.RateLimit1d {
		builder.SetRateLimit1d(key.RateLimit1d)
	}
	if fields.RateLimit7d {
		builder.SetRateLimit7d(key.RateLimit7d)
	}
