...
func (r *accountRepository) Create(ctx context.Context, account *service.Account) error {
	if err := createAccountRecord(ctx, r.client, r.sql, account); err != nil {
		return err
	}
...
func createAccountRecord(ctx context.Context, client *dbent.Client, sqlq sqlExecutor, account *service.Account) error {
	if account == nil {
	...
	if account.ParentAccountID == nil || projectID <= 0 {
		var err error
		projectID, err = resolveProjectIDForCreate(ctx, sqlq, account.ProjectID)
		if err != nil {
			return err
		}
	}
...
func (r *accountRepository) CreateWithAccountGroups(ctx context.Context, account *service.Account, groups []service.AccountGroup) error {
	...
	if err := createAccountRecord(ctx, txClient, txClient, account); err != nil {
		return err
	}
...
func (r *accountRepository) GetByIDs(ctx context.Context, ids []int64) ([]*service.Account, error) {
	...
	entAccounts, err := r.client.Account.
		Query().
		Where([]dbpredicate.Account{dbaccount.IDIn(uniqueIDs...)}...).
		WithProxy().
		All(ctx)
	if err != nil {
		return nil, err
...
func (r *accountRepository) ExistsByID(ctx context.Context, id int64) (bool, error) {
	exists, err := r.client.Account.Query().
		Where([]dbpredicate.Account{dbaccount.IDEQ(id)}...).
		Exist(ctx)
	if err != nil {
		return false, err
	}
...
func (r *accountRepository) ListOpsAccountsForStats(ctx context.Context, platformFilter string, groupIDFilter *int64) ([]service.Account, error) {
	...
	preds := []dbpredicate.Account{dbaccount.DeletedAtIsNil()}
	q := r.client.Account.Query().Where(preds...)
...
func (r *accountRepository) AddToGroup(ctx context.Context, accountID, groupID int64, priority int) error {
	...
	if _, err := r.client.Group.Query().
		Where([]dbpredicate.Group{dbgroup.IDEQ(groupID)}...).
		Only(ctx); err != nil {
...
func (r *accountRepository) GetGroups(ctx context.Context, accountID int64) ([]service.Group, error) {
	groups, err := r.client.Group.Query().
		Where([]dbpredicate.Group{dbgroup.HasAccountsWith(dbaccount.IDEQ(accountID))}...).
		All(ctx)
	if err != nil {
		return nil, err
	}
...
func (r *accountRepository) BindGroups(ctx context.Context, accountID int64, groupIDs []int64) error {
	...
	if len(groupIDs) > 0 {
		count, err := r.client.Group.Query().
			Where([]dbpredicate.Group{dbgroup.IDIn(groupIDs...)}...).
			Count(ctx)
		if err != nil {
			return err
		}
...
func (r *accountRepository) FindByExtraField(ctx context.Context, key string, value any) ([]service.Account, error) {
	preds := []dbpredicate.Account{
		...
		func(s *entsql.Selector) {
			...
			switch v := value.(type) {
			case string:
				valuePredicates := []*entsql.Predicate{sqljson.ValueEQ(dbaccount.FieldExtra, v, path)}
				if parsed, err := strconv.ParseInt(v, 10, 64); err == nil {
					valuePredicates = append(valuePredicates, sqljson.ValueEQ(dbaccount.FieldExtra, parsed, path))
				}
				if len(valuePredicates) == 1 {
					s.Where(valuePredicates[0])
				} else {
					s.Where(entsql.Or(valuePredicates...))
				}
			case int:
				s.Where(entsql.Or(
