...
func (s *UserRepoSuite) TestIncrementTokenVersionPersistsAtomically() {
	...
	for i := 0; i < 2; i++ {
		...
		go func() {
			defer wg.Done()
			errs <- s.repo.IncrementTokenVersion(context.Background(), user.ID)
		}()
	}
	wg.Wait()
	close(errs)
	...
	updated, err := s.repo.GetByID(s.ctx, user.ID)
	s.Require().NoError(err)
	s.Equal(int64(2), updated.TokenVersion)
}
