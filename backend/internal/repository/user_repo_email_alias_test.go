...
func TestUserRepositoryExistsByEmailAliasFindsDuplicateBeyondFirstCandidatePage(t *testing.T) {
	repo, _ := newUserEntRepo(t)
	...
	seedUserForAliasTest(t, repo, local+"+tag@qq.com")

	got, err := repo.ExistsByEmailAlias(context.Background(), local+"@qq.com")
	require.NoError(t, err)
	require.True(t, got)
}
