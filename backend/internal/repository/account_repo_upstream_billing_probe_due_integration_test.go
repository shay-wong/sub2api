...
func TestListDueUpstreamBillingProbeAccountsHandlesInvalidCalendarDate(t *testing.T) {
	...
	insert := func(name, nextProbeAt string) int64 {
		...
		err := scanSingleRow(ctx, tx, `
			...
			VALUES ($1, $2, 'openai', $3, 'active', $4::jsonb)
			RETURNING id
		`, []any{projectID, name, service.AccountTypeAPIKey, extra}, &id)
		require.NoError(t, err)
		return id
	}
...
func insertUpstreamBillingProbeAccount(ctx context.Context, t *testing.T, tx sqlQueryer, projectID int64, name, nextProbeAt string) int64 {
	t.Helper()
	var id int64
	extra := fmt.Sprintf(`{
		"upstream_billing_probe_enabled": true,
...
func TestListDueUpstreamBillingProbeAccountsParsesNanosecondTimestamps(t *testing.T) {
	...
	for i := 0; i < 22; i++ {
		insertUpstreamBillingProbeAccount(ctx, t, tx, projectID,
	...
	dueThird := insertUpstreamBillingProbeAccount(ctx, t, tx, projectID,
		"probe-nano-due-9digits", rfc3339WithFraction(now.Add(-time.Minute), "123456789"))
	dueFirst := insertUpstreamBillingProbeAccount(ctx, t, tx, projectID,
		"probe-nano-due-8digits", rfc3339WithFraction(now.Add(-3*time.Minute), "12345678"))
	dueSecond := insertUpstreamBillingProbeAccount(ctx, t, tx, projectID,
...
func TestListDueUpstreamBillingProbeAccountsSelectsEarliestDueAcrossIDs(t *testing.T) {
	...
	for i := 0; i < 25; i++ {
		...
		ids = append(ids, insertUpstreamBillingProbeAccount(ctx, t, tx, projectID,
