...
func TestBuildOpsErrorLogsWhere_MatchDeletedKeyOwner(t *testing.T) {
	uid := int64(42)
	filter := &service.OpsErrorLogFilter{UserID: &uid, MatchDeletedKeyOwner: true}
	where, args := buildOpsErrorLogsWhere(filter)
	if !strings.Contains(where, "(e.user_id = $1 OR e.deleted_key_owner_user_id = $1)") {
		t.Fatalf("MatchDeletedKeyOwner=true should widen to OR, got: %s", where)
	}
	if len(args) != 1 || args[0] != uid {
		t.Fatalf("expected single reused arg %d, got %v", uid, args)
	}
}
