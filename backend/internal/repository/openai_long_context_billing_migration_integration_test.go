...
VALUES ('migration-175-project', 'migration-175-project', 'Migration 175 integration test.', 'active', '{}'::jsonb, NOW(), NOW())
RETURNING id
`).Scan(&projectID))
...
VALUES ($1, 'migration-175-ordinary', 'openai', 'oauth', '{}'::jsonb)
RETURNING id
`, projectID).Scan(&ordinaryID))

	var parentID int64
	require.NoError(t, tx.QueryRowContext(ctx, `
INSERT INTO accounts (project_id, name, platform, type, extra)
VALUES ($1, 'migration-175-parent', 'openai', 'oauth', '{"openai_long_context_billing_enabled":false}'::jsonb)
RETURNING id
`, projectID).Scan(&parentID))

	var shadowID int64
	require.NoError(t, tx.QueryRowContext(ctx, `
INSERT INTO accounts (project_id, name, platform, type, extra, parent_account_id, quota_dimension)
VALUES ($1, 'migration-175-shadow', 'openai', 'oauth', '{}'::jsonb, $2, 'spark')
RETURNING id
`, projectID, parentID).Scan(&shadowID))

	var malformedLegacyID int64
	require.NoError(t, tx.QueryRowContext(ctx, `
INSERT INTO accounts (project_id, name, platform, type, extra)
VALUES ($1, 'migration-175-malformed-legacy', 'openai', 'oauth', '{"openai_long_context_billing_enabled":"false"}'::jsonb)
RETURNING id
`, projectID).Scan(&malformedLegacyID))
...
`, shadowID).Scan(&preservedOptOutEvents))
	...
	require.NoError(t, tx.QueryRowContext(ctx, `
INSERT INTO accounts (project_id, name, platform, type, extra)
VALUES ($1, 'migration-175-rolling-writer', 'openai', 'oauth', '{}'::jsonb)
RETURNING (extra->>'openai_long_context_billing_enabled')::boolean
`, projectID).Scan(&ordinaryEnabled))
	require.False(t, ordinaryEnabled)
...
`, shadowID).Scan(&shadowOutboxEvents))
	...
	_, err = tx.ExecContext(ctx, `
INSERT INTO accounts (project_id, name, platform, type, extra)
VALUES ($1, 'migration-175-malformed', 'openai', 'oauth', '{"openai_long_context_billing_enabled":"false"}'::jsonb)
`, projectID)
