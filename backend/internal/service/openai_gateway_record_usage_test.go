...
type openAIRecordUsageQuotaCacheStub struct {
	...
	subUsageCalls          int
	lastSubUsageUserID     int64
	lastSubUsageGroupID    int64
	lastSubUsageCostAmount float64
}
...
func (s *openAIRecordUsageQuotaCacheStub) UpdateSubscriptionUsage(ctx context.Context, userID, groupID int64, cost float64) error {
	s.subUsageCalls++
	s.lastSubUsageUserID = userID
	s.lastSubUsageGroupID = groupID
	s.lastSubUsageCostAmount = cost
	return nil
}
...
type openAIRecordUsageSubRepoStub struct {
	...
	activeErr       error
	activeCalls     int
	lastActiveUser  int64
	lastActiveGroup int64
}
...
func (s *openAIRecordUsageSubRepoStub) GetActiveByUserIDAndGroupID(ctx context.Context, userID, groupID int64) (*UserSubscription, error) {
	s.activeCalls++
	s.lastActiveUser = userID
	s.lastActiveGroup = groupID
	if s.activeErr != nil {
...
func TestOpenAIGatewayServiceRecordUsage_UsageLogUsesAPIKeyProject(t *testing.T) {
	usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
	billingRepo := &openAIRecordUsageBillingRepoStub{result: &UsageBillingApplyResult{Applied: true}}
	svc := newOpenAIRecordUsageServiceWithBillingRepoForTest(usageRepo, billingRepo, &openAIRecordUsageUserRepoStub{}, &openAIRecordUsageSubRepoStub{}, nil)
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		...
		APIKey:  &APIKey{ID: 1001, ProjectID: 169, Group: &Group{RateMultiplier: 1}},
		User:    &User{ID: 2001},
	...
	require.NoError(t, err)
	require.NotNil(t, usageRepo.lastLog)
	require.Equal(t, int64(169), usageRepo.lastLog.ProjectID)
}
...
func TestOpenAIGatewayServiceRecordUsage_UsesActualSelectionGroupForRateAndPeak(t *testing.T) {
	apiKeyGroupID := int64(11)
	actualGroupID := int64(22)
	userRate := 1.7
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID: "resp_actual_group_rate",
			Usage:     usage,
			Model:     "gpt-5.1",
			Duration:  time.Second,
		},
		APIKey: &APIKey{
			...
			Group: &Group{
				...
				PeakStart:          "00:00",
				PeakEnd:            "23:59",
				PeakRateMultiplier: 9.0,
			},
		},
		User:             &User{ID: 2002},
		Account:          &Account{ID: 3002},
		EffectiveGroupID: i64p(actualGroupID),
		EffectiveGroup:   actualGroup,
	})

	require.NoError(t, err)
	require.Equal(t, 1, rateRepo.calls)
	require.Equal(t, int64(2002), rateRepo.lastUserID)
	require.Equal(t, actualGroupID, rateRepo.lastGroupID)
	require.NotNil(t, usageRepo.lastLog)
	require.InDelta(t, userRate*actualGroup.PeakRateMultiplier, usageRepo.lastLog.RateMultiplier, 1e-12)

	expected := expectedOpenAICost(t, svc, "gpt-5.1", usage, userRate*actualGroup.PeakRateMultiplier)
	require.InDelta(t, expected.ActualCost, usageRepo.lastLog.ActualCost, 1e-12)
	require.InDelta(t, expected.ActualCost, userRepo.lastAmount, 1e-12)
}

func TestOpenAIGatewayServiceRecordUsage_ActualSubscriptionGroupControlsBillingMode(t *testing.T) {
	apiKeyGroupID := int64(31)
	actualGroupID := int64(41)
	userID := int64(2401)
	actualSubscription := &UserSubscription{ID: 4101, UserID: userID, GroupID: actualGroupID}
	usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
	billingRepo := &openAIRecordUsageBillingRepoStub{result: &UsageBillingApplyResult{Applied: true}}
	subRepo := &openAIRecordUsageSubRepoStub{
		activeByGroup: map[int64]*UserSubscription{actualGroupID: actualSubscription},
	}
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID: "resp_actual_subscription_group",
			Usage:     OpenAIUsage{InputTokens: 10, OutputTokens: 5},
			Model:     "gpt-5.1",
			Duration:  time.Second,
		},
		APIKey: &APIKey{
			ID:      3101,
			GroupID: i64p(apiKeyGroupID),
			Group:   &Group{ID: apiKeyGroupID, SubscriptionType: SubscriptionTypeStandard, RateMultiplier: 1.0},
		},
		User:             &User{ID: userID},
		Account:          &Account{ID: 3201},
		Subscription:     &UserSubscription{ID: 3102, UserID: userID, GroupID: apiKeyGroupID},
		EffectiveGroupID: i64p(actualGroupID),
		EffectiveGroup:   &Group{ID: actualGroupID, SubscriptionType: SubscriptionTypeSubscription, RateMultiplier: 1.0},
	})

	require.NoError(t, err)
	require.Equal(t, 1, subRepo.activeCalls)
	require.Equal(t, userID, subRepo.lastActiveUser)
	require.Equal(t, actualGroupID, subRepo.lastActiveGroup)
	require.NotNil(t, usageRepo.lastLog)
	require.Equal(t, BillingTypeSubscription, usageRepo.lastLog.BillingType)
	require.NotNil(t, usageRepo.lastLog.GroupID)
	require.Equal(t, actualGroupID, *usageRepo.lastLog.GroupID)
	require.NotNil(t, usageRepo.lastLog.SubscriptionID)
	require.Equal(t, actualSubscription.ID, *usageRepo.lastLog.SubscriptionID)
	require.NotNil(t, billingRepo.lastCmd)
	...
	require.Equal(t, actualSubscription.ID, *billingRepo.lastCmd.SubscriptionID)
	require.Greater(t, billingRepo.lastCmd.SubscriptionCost, 0.0)
	require.Zero(t, billingRepo.lastCmd.BalanceCost)
	require.Equal(t, 1, quotaCache.subUsageCalls)
	require.Equal(t, userID, quotaCache.lastSubUsageUserID)
	require.Equal(t, actualGroupID, quotaCache.lastSubUsageGroupID)
	require.InDelta(t, billingRepo.lastCmd.SubscriptionCost, quotaCache.lastSubUsageCostAmount, 1e-12)
}

func TestOpenAIGatewayServiceRecordUsage_ActualStandardGroupControlsBillingMode(t *testing.T) {
	apiKeyGroupID := int64(51)
	actualGroupID := int64(61)
	userID := int64(2601)
	usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
	billingRepo := &openAIRecordUsageBillingRepoStub{result: &UsageBillingApplyResult{Applied: true}}
	subRepo := &openAIRecordUsageSubRepoStub{}
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID: "resp_actual_standard_group",
			Usage:     OpenAIUsage{InputTokens: 10, OutputTokens: 5},
			Model:     "gpt-5.1",
			Duration:  time.Second,
		},
		APIKey: &APIKey{
			ID:      5101,
			GroupID: i64p(apiKeyGroupID),
			Group:   &Group{ID: apiKeyGroupID, SubscriptionType: SubscriptionTypeSubscription, RateMultiplier: 1.0},
		},
		User:             &User{ID: userID},
		Account:          &Account{ID: 5201},
		Subscription:     &UserSubscription{ID: 5102, UserID: userID, GroupID: apiKeyGroupID},
		EffectiveGroupID: i64p(actualGroupID),
		EffectiveGroup:   &Group{ID: actualGroupID, SubscriptionType: SubscriptionTypeStandard, RateMultiplier: 1.0},
	})

	require.NoError(t, err)
	require.Equal(t, 0, subRepo.activeCalls)
	require.NotNil(t, usageRepo.lastLog)
	...
	require.Zero(t, billingRepo.lastCmd.SubscriptionCost)
	require.Equal(t, 0, quotaCache.subUsageCalls)
}
...
func TestOpenAIGatewayServiceRecordUsage_BillingCommandUsesExplicitEffectiveGroup(t *testing.T) {
	apiKeyGroupID := int64(30)
	actualGroupID := int64(40)
	usageRepo := &openAIRecordUsageLogRepoStub{}
	billingRepo := &openAIRecordUsageBillingRepoStub{result: &UsageBillingApplyResult{Applied: true}}
...
func TestOpenAIGatewayServiceRecordUsage_FallbackQuotaPlatformUsesExplicitEffectiveGroup(t *testing.T) {
	apiKeyGroupID := int64(30)
	actualGroupID := int64(40)
	dailyLimit := 1.0
	usageRepo := &openAIRecordUsageLogRepoStub{}
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID: "openai_actual_group_quota_platform",
			Usage: OpenAIUsage{
				InputTokens:  1000,
				OutputTokens: 200,
			},
	...
	require.NoError(t, err)
	require.Equal(t, 1, quotaCache.incrCalls)
	require.Equal(t, int64(2202), quotaCache.lastIncrUserID)
	require.Equal(t, PlatformAntigravity, quotaCache.lastIncrPlatform)
	require.Greater(t, quotaCache.lastIncrCost, 0.0)
	require.True(t, quotaCache.lastIncrMarkDirty)
}

func TestOpenAIGatewayServiceRecordUsage_ImageCostUsesExplicitEffectiveGroup(t *testing.T) {
	apiKeyGroupID := int64(30)
	actualGroupID := int64(40)
	apiKeyGroupImagePrice := 0.99
	actualGroupImagePrice := 0.021
	usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
	billingRepo := &openAIRecordUsageBillingRepoStub{result: &UsageBillingApplyResult{Applied: true}}
	svc := newOpenAIRecordUsageServiceWithBillingRepoForTest(usageRepo, billingRepo, &openAIRecordUsageUserRepoStub{}, &openAIRecordUsageSubRepoStub{}, nil)
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID:    "openai_actual_group_image_price",
			Model:        "gpt-image-2",
		...
		APIKey: &APIKey{
			...
			Group: &Group{
				...
				RateMultiplier: 1,
				ImagePrice2K:   &apiKeyGroupImagePrice,
			},
		},
		User:             &User{ID: 2203},
		Account:          &Account{ID: 3203},
		EffectiveGroupID: &actualGroupID,
		EffectiveGroup: &Group{
			ID:             actualGroupID,
			Platform:       PlatformAntigravity,
			RateMultiplier: 1,
			ImagePrice2K:   &actualGroupImagePrice,
		},
	})
	...
	require.NotNil(t, usageRepo.lastLog.GroupID)
	require.Equal(t, actualGroupID, *usageRepo.lastLog.GroupID)
	require.InDelta(t, actualGroupImagePrice, usageRepo.lastLog.ActualCost, 1e-12)
	require.NotEqual(t, apiKeyGroupImagePrice, usageRepo.lastLog.ActualCost)
	require.NotNil(t, usageRepo.lastLog.BillingMode)
	require.Equal(t, string(BillingModeImage), *usageRepo.lastLog.BillingMode)
}

func TestOpenAIGatewayServiceRecordUsage_VideoCostUsesExplicitEffectiveGroup(t *testing.T) {
	apiKeyGroupID := int64(31)
	actualGroupID := int64(41)
	apiKeyGroupVideoPrice := 0.44
	actualGroupVideoPrice := 0.08
	usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
	billingRepo := &openAIRecordUsageBillingRepoStub{result: &UsageBillingApplyResult{Applied: true}}
	svc := newOpenAIRecordUsageServiceWithBillingRepoForTest(usageRepo, billingRepo, &openAIRecordUsageUserRepoStub{}, &openAIRecordUsageSubRepoStub{}, nil)
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID:            "openai_actual_group_video_price",
		...
		APIKey: &APIKey{
			...
			Group: &Group{
				...
				VideoPrice480P:       &apiKeyGroupVideoPrice,
			},
		},
		User:             &User{ID: 2204},
		Account:          &Account{ID: 3204, Platform: PlatformGrok},
		EffectiveGroupID: &actualGroupID,
		EffectiveGroup: &Group{
			ID:                   actualGroupID,
			Platform:             PlatformGrok,
			...
			VideoPrice480P:       &actualGroupVideoPrice,
		},
	})
	...
	require.NotNil(t, usageRepo.lastLog.GroupID)
	require.Equal(t, actualGroupID, *usageRepo.lastLog.GroupID)
	require.InDelta(t, actualGroupVideoPrice, usageRepo.lastLog.TotalCost, 1e-12)
	require.InDelta(t, actualGroupVideoPrice*0.25, usageRepo.lastLog.ActualCost, 1e-12)
	require.InDelta(t, 0.25, usageRepo.lastLog.RateMultiplier, 1e-12)
	require.NotEqual(t, apiKeyGroupVideoPrice*0.9, usageRepo.lastLog.ActualCost)
	require.NotNil(t, usageRepo.lastLog.BillingMode)
	require.Equal(t, string(BillingModeVideo), *usageRepo.lastLog.BillingMode)
	require.NotNil(t, billingRepo.lastCmd)
}

func TestOpenAIGatewayServiceRecordUsage_UsageLogFallsBackToAPIKeyGroup(t *testing.T) {
	apiKeyGroupID := int64(30)
	usageRepo := &openAIRecordUsageLogRepoStub{}
	billingRepo := &openAIRecordUsageBillingRepoStub{result: &UsageBillingApplyResult{Applied: true}}
	svc := newOpenAIRecordUsageServiceWithBillingRepoForTest(usageRepo, billingRepo, &openAIRecordUsageUserRepoStub{}, &openAIRecordUsageSubRepoStub{}, nil)
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID: "openai_api_key_group",
			Usage: OpenAIUsage{
				InputTokens:  1000,
				OutputTokens: 200,
			},
