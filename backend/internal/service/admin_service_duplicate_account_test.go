...
func (s *duplicateAccountGroupRepoStub) ExistsByIDs(_ context.Context, ids []int64) (map[int64]bool, error) {
	existing := make(map[int64]bool, len(ids))
	for _, id := range ids {
		existing[id] = id > 0 && !s.missing[id]
	}
	return existing, nil
}

type duplicateAccountProxyRepoStub struct {
	ProxyRepository
	err error
}

func (s *duplicateAccountProxyRepoStub) GetByID(context.Context, int64) (*Proxy, error) {
	return nil, s.err
}

func newDuplicateAccountService(repo *duplicateAccountRepoStub) *adminServiceImpl {
	return &adminServiceImpl{
		accountRepo:          repo,
		accountDuplicateRepo: repo,
		groupRepo:            &duplicateAccountGroupRepoStub{},
	}
}
...
func TestDuplicateAccountRejectsResourcesOutsideCurrentProjectScope(t *testing.T) {
	t.Run("group", func(t *testing.T) {
		ctx := context.Background()
		repo := newDuplicateAccountRepoStub()
		svc := newDuplicateAccountService(repo)
		svc.groupRepo = &duplicateAccountGroupRepoStub{missing: map[int64]bool{99: true}}
		source := &Account{
			Name: "source", Platform: PlatformAnthropic, Type: AccountTypeAPIKey,
			Status: StatusActive, Credentials: map[string]any{"api_key": "secret"}, GroupIDs: []int64{99},
		}
		require.NoError(t, repo.Create(ctx, source))
	...
	t.Run("proxy", func(t *testing.T) {
		...
		repo := newDuplicateAccountRepoStub()
		svc := newDuplicateAccountService(repo)
		svc.proxyRepo = &duplicateAccountProxyRepoStub{err: ErrProxyNotFound}
		proxyID := int64(17)
		source := &Account{
			Name: "source", Platform: PlatformAnthropic, Type: AccountTypeAPIKey,
			Status: StatusActive, Credentials: map[string]any{"api_key": "secret"}, ProxyID: &proxyID,
		}
		require.NoError(t, repo.Create(ctx, source))
