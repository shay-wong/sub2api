...
func TestAccountSupportsOpenAIEndpointCapability(t *testing.T) {
	...
	t.Run("OpenAI OAuth 默认兼容 chat 和 alpha search", func(t *testing.T) {
		account := &Account{
			Platform: PlatformOpenAI,
			Type:     AccountTypeOAuth,
		}
	...
	t.Run("自定义 OpenAI APIKey 未声明时默认尝试 alpha search", func(t *testing.T) {
		account := &Account{
			Platform: PlatformOpenAI,
			Type:     AccountTypeAPIKey,
	...
	t.Run("自定义 OpenAI APIKey 可显式声明 alpha search", func(t *testing.T) {
		account := &Account{
			Platform: PlatformOpenAI,
			Type:     AccountTypeAPIKey,
			Credentials: map[string]any{
	...
	t.Run("OpenAI APIKey 的 chat 能力同时放行 alpha search", func(t *testing.T) {
		account := &Account{
			Platform: PlatformOpenAI,
			Type:     AccountTypeAPIKey,
			Credentials: map[string]any{
