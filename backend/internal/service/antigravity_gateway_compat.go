...
func (s *AntigravityGatewayService) forwardAntigravityCompat(
	...
	upstreamRequestCtx := ctx
	var cancelUpstream context.CancelFunc
	var stopClientCancellation func() bool
	if request.clientStream {
		upstreamRequestCtx, cancelUpstream = context.WithCancel(context.WithoutCancel(ctx))
		stopClientCancellation = context.AfterFunc(ctx, cancelUpstream)
		defer cancelUpstream()
	}

	result, err := s.antigravityRetryLoop(antigravityRetryLoopParams{
		ctx:                ctx,
		upstreamRequestCtx: upstreamRequestCtx,
		prefix:             call.prefix,
		account:            account,
	...
	if request.clientStream && result != nil && result.resp != nil && result.resp.StatusCode < http.StatusBadRequest {
		if !stopClientCancellation() {
			_ = result.resp.Body.Close()
			cancelErr := ctx.Err()
			if cancelErr == nil {
				cancelErr = context.Canceled
			}
			return nil, s.handleAntigravityCompatTransportError(c, cancelErr)
		}
	}
