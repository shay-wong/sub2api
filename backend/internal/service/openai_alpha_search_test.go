...
func TestForwardAlphaSearchReturnsFailoverBeforeWriting(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "model missing", statusCode: http.StatusNotFound, body: `{"error":{"code":"model_not_found","message":"model not found"}}`, neutral: true},
...
func TestForwardAlphaSearchModelNotFoundIsNeutralForAPIKeyHealth(t *testing.T) {
	...
	var failoverErr *UpstreamFailoverError
	require.ErrorAs(t, err, &failoverErr)
	require.True(t, failoverErr.NeutralForAccountHealth)
	require.Zero(t, repo.tempUnschedulableCalls)
	require.Zero(t, repo.modelRateLimitCalls)
}
