...
type alphaSearchFailoverAccountRepo struct {
	AccountRepository
	tempUnschedulableCalls int
	modelRateLimitCalls    int
}

func (r *alphaSearchFailoverAccountRepo) SetTempUnschedulable(context.Context, int64, time.Time, string) error {
	r.tempUnschedulableCalls++
	return nil
}

func (r *alphaSearchFailoverAccountRepo) SetModelRateLimit(context.Context, int64, string, time.Time, ...string) error {
...
func TestForwardAlphaSearchReturnsFailoverBeforeWriting(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "endpoint missing", statusCode: http.StatusNotFound, body: `{"error":{"message":"not found"}}`, neutral: true},
		{name: "model missing", statusCode: http.StatusNotFound, body: `{"error":{"code":"model_not_found","message":"model not found"}}`},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, tc.neutral, failoverErr.NeutralForAccountHealth)
...
func TestForwardAlphaSearchEndpointUnavailableDoesNotMutateAccountHealth(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body:       io.NopCloser(strings.NewReader(`{"error":{"message":"endpoint not found"}}`)),
	}}
	repo := &alphaSearchFailoverAccountRepo{}
...
func TestForwardAlphaSearchModelNotFoundUpdatesModelHealth(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"id":"search-session","model":"gpt-5.6-sol","commands":{}}`)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	...
	var failoverErr *UpstreamFailoverError
	require.ErrorAs(t, err, &failoverErr)
	require.False(t, failoverErr.NeutralForAccountHealth)
	require.Zero(t, repo.tempUnschedulableCalls)
