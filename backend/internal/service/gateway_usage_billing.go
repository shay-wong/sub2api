...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput) error {
	...
	multiplier, imageMultiplier := computePeakAwareMultipliers(billingAPIKey, multiplier, pricingAt)
	...
	if billingAPIKey.Group != nil && billingAPIKey.Group.Platform == PlatformComposite {
		billingModel = s.compositeBillableModel(ctx, billingAPIKey, billingModel, concreteBillingModel)
	}
	// 通用兜底（与 OpenAI 路径的 usageBillingModelCandidates 语义对齐）：
	// 选定模型查不到任何价格时回退到实际转发的具体模型。已定价流量不受影响。
	billingModel = s.billableModelWithFallback(ctx, billingAPIKey, billingModel, result.UpstreamModel, result.Model)
	...
	// 计算费用
	cost := s.calculateRecordUsageCost(ctx, result, billingAPIKey, billingModel, multiplier, imageMultiplier, pricingAt)
	...
	if responseModel := responseModelBillingDeclaration(
		...
		if identified, responseChannelPriced := s.hasIdentifiedResponseModelPricing(ctx, responseModel, billingAPIKey); identified {
			responseCost := s.calculateRecordUsageCost(ctx, result, billingAPIKey, responseModel, multiplier, imageMultiplier, pricingAt)
	...
	usageLog := s.buildRecordUsageLog(ctx, input, result, apiKey, user, account, subscription,
		requestedModel, multiplier, imageMultiplier, accountRateMultiplier, billingType, cacheTTLOverridden, cost, billingGroupID)
