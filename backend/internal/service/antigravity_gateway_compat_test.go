...
type antigravityCompatFailingAdapter struct {
	err error
}

func (a antigravityCompatFailingAdapter) Emit(*apicompat.AnthropicStreamEvent, *antigravityClientWriter) error {
	return a.err
}

func (a antigravityCompatFailingAdapter) Finalize(*antigravityClientWriter) error {
	return a.err
}

func (antigravityCompatFailingAdapter) WriteError(*antigravityClientWriter, string) {}

func (antigravityCompatFailingAdapter) Completed() bool { return false }
...
func antigravityCompatClientToolMapping() apicompat.ResponsesClientToolMapping {
...
func antigravityCompatToolCallResponse() *http.Response {
	body := `data: {"response":{"responseId":"resp_tools","candidates":[{"content":{"parts":[` +
		`{"functionCall":{"id":"call_exec","name":"exec","args":{"input":"pwd"}}},` +
		`{"functionCall":{"id":"call_search","name":"tool_search","args":{"query":"git"}}},` +
		`{"functionCall":{"id":"call_namespace","name":"collaboration__send_message","args":{"target":"root"}}}` +
		`]},"finishReason":"STOP"}],"usageMetadata":{"promptTokenCount":8,"candidatesTokenCount":3}}}` + "\n\n"
	return &http.Response{
		StatusCode: http.StatusOK,
...
// Codex-only tool declarations and history must be lowered before the shared
// Responses-to-Anthropic conversion reaches Antigravity.
func TestAntigravityCompatResponsesAdaptsClientTools(t *testing.T) {
	body := []byte(`{
		"model":"claude-sonnet-4-5",
		"input":[
			{"type":"message","role":"user","content":[{"type":"input_text","text":"use tools"}]},
			{"type":"additional_tools","role":"developer","tools":[{"type":"custom","name":"patch","description":"apply patch"}]},
			{"type":"custom_tool_call","call_id":"call_exec_old","name":"exec","input":"pwd"},
			{"type":"custom_tool_call_output","call_id":"call_exec_old","output":"ok"},
			{"type":"tool_search_call","call_id":"call_search_old","arguments":{"query":"git"},"execution":"client"},
			{"type":"tool_search_output","call_id":"call_search_old","output":{"groups":["git"]}},
			{"type":"function_call","call_id":"call_ns_old","namespace":"collaboration","name":"send_message","arguments":"{}"},
			{"type":"function_call_output","call_id":"call_ns_old","output":"ok"}
		],
		"tools":[
			{"type":"custom","name":"exec","description":"execute"},
			{"type":"tool_search"},
	...
	require.Contains(t, forwarded, `"exec"`)
	require.Contains(t, forwarded, `"patch"`)
	require.Contains(t, forwarded, `"tool_search"`)
	require.Contains(t, forwarded, `"collaboration__send_message"`)
	require.NotContains(t, forwarded, `"additional_tools"`)
	require.NotContains(t, forwarded, `"custom_tool_call"`)
	require.NotContains(t, forwarded, `"tool_search_call"`)
	require.NotContains(t, forwarded, `"namespace":"collaboration"`)
}
...
func TestAntigravityCompatResponsesRestoresClientToolsNonStreaming(t *testing.T) {
	svc := newAntigravityCompatService(config.GatewayConfig{MaxLineSize: defaultMaxLineSize}, nil)
	c, recorder := newAntigravityCompatContext(http.MethodPost, "/v1/responses", nil)

	result, err := svc.handleResponsesNonStreamingFromAntigravity(
		c,
		antigravityCompatToolCallResponse(),
		time.Now(),
		"claude-sonnet-4-5",
		antigravityCompatClientToolMapping(),
	)

	require.NoError(t, err)
	require.NotNil(t, result)
	payload := recorder.Body.String()
	require.Equal(t, "exec", gjson.Get(payload, `output.#(type=="custom_tool_call").name`).String())
	require.Equal(t, "pwd", gjson.Get(payload, `output.#(type=="custom_tool_call").input`).String())
	require.Equal(t, "client", gjson.Get(payload, `output.#(type=="tool_search_call").execution`).String())
	require.Equal(t, "send_message", gjson.Get(payload, `output.#(namespace=="collaboration").name`).String())
	require.NotContains(t, payload, `"name":"collaboration__send_message"`)
}

func TestAntigravityCompatResponsesRestoresClientToolsStreaming(t *testing.T) {
	svc := newAntigravityCompatService(config.GatewayConfig{MaxLineSize: defaultMaxLineSize}, nil)
	c, recorder := newAntigravityCompatContext(http.MethodPost, "/v1/responses", nil)

	result, err := svc.handleResponsesStreamingFromAntigravity(
		c,
		antigravityCompatToolCallResponse(),
		time.Now(),
		"claude-sonnet-4-5",
	...
	require.Contains(t, stream, `"type":"response.custom_tool_call_input.done"`)
	require.Contains(t, stream, `"type":"custom_tool_call"`)
	require.Contains(t, stream, `"type":"tool_search_call"`)
	require.Contains(t, stream, `"namespace":"collaboration"`)
	require.NotContains(t, stream, `"name":"collaboration__send_message"`)
}

func TestAntigravityCompatStreamSessionPropagatesConversionErrors(t *testing.T) {
	sentinel := errors.New("conversion failed")
	session := newAntigravityCompatStreamSession(
		"claude-sonnet-4-5",
		time.Now(),
		antigravityCompatFailingAdapter{err: sentinel},
		&antigravityClientWriter{},
	)

	err := session.emitOrBuffer(apicompat.AnthropicStreamEvent{
		Type:  "content_block_delta",
		Delta: &apicompat.AnthropicDelta{Type: "text_delta", Text: "hello"},
	})

	require.ErrorIs(t, err, sentinel)
	require.Error(t, session.consumeClaudeEvents([]byte("event: message_start\ndata: not-json\n")))
}

func TestAntigravityCompatResponsesRejectsCorruptedPartialStream(t *testing.T) {
	tests := []struct {
		name string
		tail string
	}{
		{name: "malformed_json", tail: `data: not-json`},
		{name: "upstream_error", tail: `data: {"error":{"message":"upstream overloaded"}}`},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			svc := newAntigravityCompatService(config.GatewayConfig{MaxLineSize: defaultMaxLineSize}, nil)
...
func TestAntigravityCompatFirstConversionErrorAllowsFailoverBeforeClientWrite(t *testing.T) {
	sentinel := errors.New("conversion failed")
	...
	result, err := svc.handleAntigravityCompatConversionError(c, session, nil, sentinel, "test")
...
func TestAntigravityCompatRequestCancellationDrainsAcceptedStreamUsage(t *testing.T) {
	reader, writer := io.Pipe()
	upstreamRequestCtx := make(chan context.Context, 1)
	upstream := &queuedHTTPUpstreamStub{
		...
		onCall: func(req *http.Request, _ *queuedHTTPUpstreamStub) {
			upstreamRequestCtx <- req.Context()
			go func() {
				<-req.Context().Done()
	...
	svc := newAntigravityCompatService(config.GatewayConfig{MaxLineSize: defaultMaxLineSize}, upstream)
	body := []byte(`{"model":"gemini-3.1-pro-high","messages":[{"role":"user","content":"ok"}],"stream":true}`)
	c, _ := newAntigravityCompatContext(http.MethodPost, "/v1/chat/completions", body)
	...
	go func() {
		result, err := svc.ForwardAsChatCompletions(requestCtx, c, newAntigravityCompatAccount(AccountTypeOAuth), body, nil)
		done <- outcome{result: result, err: err}
	}()

	acceptedRequestCtx := <-upstreamRequestCtx
	_, err := io.WriteString(writer,
		`data: {"response":{"responseId":"resp_cancel","candidates":[{"content":{"parts":[{"text":"partial"}]}}],"usageMetadata":{"promptTokenCount":8,"candidatesTokenCount":1}}}`+"\n\n",
	)
	require.NoError(t, err)

	cancel()
	select {
	case <-acceptedRequestCtx.Done():
		t.Fatal("accepted upstream stream was canceled with the client request")
	case <-time.After(20 * time.Millisecond):
	}
	_, err = io.WriteString(writer,
		`data: {"response":{"responseId":"resp_cancel","candidates":[{"finishReason":"STOP"}],"usageMetadata":{"promptTokenCount":8,"candidatesTokenCount":15}}}`+"\n\n",
	)
	require.NoError(t, err)
...
func TestAntigravityCompatDisabledStreamTimeoutStillHasDisconnectDeadline(t *testing.T) {
	svc := newAntigravityCompatService(config.GatewayConfig{StreamDataIntervalTimeout: 0}, nil)
...
func TestAntigravityCompatResponsesTerminalStopsBeforeTrailingReadError(t *testing.T) {
	svc := newAntigravityCompatService(config.GatewayConfig{MaxLineSize: defaultMaxLineSize}, nil)
	c, recorder := newAntigravityCompatContext(http.MethodPost, "/v1/responses", nil)
	body := []byte(`data: {"response":{"responseId":"resp_terminal","candidates":[{"content":{"parts":[{"text":"done"}]},"finishReason":"STOP"}],"usageMetadata":{"promptTokenCount":8,"candidatesTokenCount":3}}}` + "\n\n")
	resp := &http.Response{
		StatusCode: http.StatusOK,
		Body: &antigravityCompatErrorReader{
			data: body,
...
func TestAntigravityCompatChatTerminalStopsBeforeTrailingReadError(t *testing.T) {
	svc := newAntigravityCompatService(config.GatewayConfig{MaxLineSize: defaultMaxLineSize}, nil)
	c, recorder := newAntigravityCompatContext(http.MethodPost, "/v1/chat/completions", nil)
	body := []byte(`data: {"response":{"responseId":"resp_terminal","candidates":[{"content":{"parts":[{"text":"done"}]},"finishReason":"STOP"}],"usageMetadata":{"promptTokenCount":8,"candidatesTokenCount":3}}}` + "\n\n")
	resp := &http.Response{
