...
type AccountSelectionResult struct {
	...
	GroupID     *int64           // resolved group used for this selection; may differ from the API key group after fallback.
	Group       *Group
...
type ForwardResult struct {
	...
	Usage     ClaudeUsage
	Model     string
	// UpstreamModel is the actual upstream model after mapping. Keep it when known even if it
	// matches the requested model; multi-stage mappings can loop back.
...
type UpstreamFailoverError struct {
	...
	NeutralForAccountHealth  bool        // 端点能力不匹配等请求级 failover，不计入账号级调度健康分
...
// ShouldReportAccountScheduleFailure prevents provider-scoped credential failures
// and request-scoped transients from being misattributed to the selected account.
// Legacy and inference failures retain their existing scheduler-health behavior.
func (e *UpstreamFailoverError) ShouldReportAccountScheduleFailure() bool {
	if e == nil {
		return false
	}
	if e.NeutralForAccountHealth || e.RequestScopedTransient {
		return false
	}
...
func (s *GatewayService) initDebugGatewayBodyFile(path string) {
	...
	// #nosec G703 -- SUB2API_DEBUG_GATEWAY_BODY is an operator-controlled debug path.
