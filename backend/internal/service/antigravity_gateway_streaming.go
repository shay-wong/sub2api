...
func (cw *antigravityClientWriter) Started() bool      { return cw.wrote }
...
func (s *AntigravityGatewayService) handleClaudeStreamingResponse(c *gin.Context, resp *http.Response, startTime time.Time, originalModel string) (*antigravityStreamResult, error) {
	...
	for {
		...
		case ev, ok := <-events:
			...
			claudeEvents, processErr := processor.ProcessLine(strings.TrimRight(ev.line, "\r\n"))
			if processErr != nil {
				if !cw.Started() {
					return nil, &UpstreamFailoverError{
						StatusCode:             http.StatusBadGateway,
				...
				sendErrorEvent("stream_conversion_error")
				return &antigravityStreamResult{usage: finishUsage(), firstTokenMs: firstTokenMs, clientDisconnect: cw.Disconnected()}, processErr
			}
