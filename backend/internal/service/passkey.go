...
func NewPasskeyService(
	...
	if identity == nil {
		return nil, fmt.Errorf("initialize passkeys: identity verification service is required")
	}
...
func (s *PasskeyService) GetVerificationMethod(ctx context.Context, userID int64) (*VerificationMethod, error) {
	...
	if passkeyUsesEmailVerification(user) {
		return &VerificationMethod{Method: "email"}, nil
	}
	return &VerificationMethod{Method: "password"}, nil
}

func (s *PasskeyService) SendVerifyCode(ctx context.Context, userID int64, locale ...string) error {
	if err := s.requireEnabled(); err != nil {
		return err
	}
	user, err := s.userRepo.GetByID(ctx, userID)
	...
	siteName := s.identity.settingService.GetSiteName(ctx)
	return s.identity.emailQueueService.EnqueueVerifyCode(user.Email, siteName, firstEmailLocale(locale))
}

func passkeyUsesEmailVerification(user *User) bool {
	return userRequiresEmailIdentityVerification(user)
}

func (s *PasskeyService) verifyIdentity(ctx context.Context, user *User, emailCode, password string) error {
	if passkeyUsesEmailVerification(user) {
		if emailCode == "" {
			return ErrVerifyCodeRequired
		}
		return s.identity.emailService.VerifyCode(ctx, user.Email, emailCode)
	}
	if password == "" {
...
func (s *PasskeyService) Delete(ctx context.Context, userID, credentialID int64, emailCode, password string) error {
	if err := s.requireEnabled(); err != nil {
		return err
	}
	user, err := s.userRepo.GetByID(ctx, userID)
