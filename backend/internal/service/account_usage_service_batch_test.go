...
func TestAccountIsSyntheticUITest(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range []struct {
		...
		{name: "nil account"},
		{name: "missing marker", account: &Account{}},
		{name: "non boolean marker", account: &Account{Extra: map[string]any{"synthetic_ui_test": "true"}}},
		{name: "enabled", account: &Account{Extra: map[string]any{"synthetic_ui_test": true}}, want: true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if got := tc.account.IsSyntheticUITest(); got != tc.want {
				t.Fatalf("IsSyntheticUITest() = %v, want %v", got, tc.want)
			}
		})
	}
}
...
func (r *usageBatchLogRepoStub) CountWithFilters(context.Context, usagestats.UsageLogFilters) (int64, error) {
	return 0, nil
}
func (r *usageBatchLogRepoStub) ListModelCandidates(context.Context, usagestats.UsageLogFilters, int) ([]string, error) {
	return nil, nil
}
