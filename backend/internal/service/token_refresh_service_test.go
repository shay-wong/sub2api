...
func TestIsNonRetryableRefreshError(t *testing.T) {
	tests := []struct {
		...
		{name: "app_session_terminated", err: errors.New(`OPENAI_OAUTH_TOKEN_REFRESH_FAILED: token refresh failed: status 400, body: {"error":{"code":"app_session_terminated"}}`), expected: true},
