...
func (s *OpenAIGatewayService) handleChatBufferedStreamingResponseWithOptions(
	...
	if strings.TrimSpace(finalResponse.Status) == "failed" {
		...
		if s.shouldFailoverOpenAIStreamTerminalEvent(account, "response.failed", payload, message) {
...
func (s *OpenAIGatewayService) handleChatStreamingResponseWithOptions(
	...
	processDataLine := func(payload string) bool {
		...
		if terminalType == "response.failed" || terminalType == "error" {
			...
			shouldFailover := s.shouldFailoverOpenAIStreamTerminalEvent(account, terminalType, payloadBytes, message)
