...
func TestAPIKeyUpdate_OnlyDeclaresRequestedColumns(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "5h rate limit only",
			req:  UpdateAPIKeyRequest{RateLimit5h: &rateLimit},
			want: APIKeyUpdateFields{RateLimit5h: true},
		},
		{
			name: "ip whitelist only",
			req:  UpdateAPIKeyRequest{IPWhitelist: &whitelist},
			want: APIKeyUpdateFields{IPWhitelist: true},
		},
		{
			name: "ip blacklist only",
			req:  UpdateAPIKeyRequest{IPBlacklist: &blacklist},
			want: APIKeyUpdateFields{IPBlacklist: true},
		},
	}
