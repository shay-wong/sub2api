...
func (s *adminServiceImpl) BulkUpdateAccounts(ctx context.Context, input *BulkUpdateAccountsInput) (*BulkUpdateAccountsResult, error) {
	...
	requiresAccountPreload := len(input.Credentials) > 0 || input.ProxyID != nil || needMixedChannelCheck || projectScopedBulkUpdate || (input.GroupIDs != nil && len(input.GroupScopeIDs) > 0)
