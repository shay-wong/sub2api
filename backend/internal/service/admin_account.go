...
func (s *adminServiceImpl) DuplicateAccount(ctx context.Context, id int64, actorScope, operationKey string) (*Account, error) {
	...
	if err := s.ensureProxyAvailable(ctx, proxyID); err != nil {
		return nil, err
	}
	if len(groupIDs) > 0 {
...
func (s *adminServiceImpl) BulkUpdateAccounts(ctx context.Context, input *BulkUpdateAccountsInput) (*BulkUpdateAccountsResult, error) {
	...
	requiresAccountPreload := len(input.Credentials) > 0 || input.ProxyID != nil || needMixedChannelCheck || hasLongContextBillingUpdate || input.ProbeEnabled != nil || input.RateMultiplier != nil || projectScopedBulkUpdate || (input.GroupIDs != nil && len(input.GroupScopeIDs) > 0)
	...
	// is also stripped when the platform is unknown).
