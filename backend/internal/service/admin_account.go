...
func (s *adminServiceImpl) BulkUpdateAccounts(ctx context.Context, input *BulkUpdateAccountsInput) (*BulkUpdateAccountsResult, error) {
	...
	requiresAccountPreload := len(input.Credentials) > 0 || input.ProxyID != nil || needMixedChannelCheck || hasLongContextBillingUpdate || input.ProbeEnabled != nil || projectScopedBulkUpdate || (input.GroupIDs != nil && len(input.GroupScopeIDs) > 0)
