...
func TestResolveOpenAIUsageLogRateMultiplierWebSearchUsesBaseMultiplier(t *testing.T) {
	result := &OpenAIForwardResult{WebSearchCalls: 1}
	cost := &CostBreakdown{BillingMode: string(BillingModePerRequest)}

	got := resolveOpenAIUsageLogRateMultiplier(result, cost, false, 6.0, 4.0, 5.0, 2.0)
