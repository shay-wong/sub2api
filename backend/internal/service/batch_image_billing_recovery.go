...
func (s *BatchImageBillingRecoveryService) ReleaseStaleUnsubmittedOnce(ctx context.Context) (int, error) {
	...
	for _, job := range jobs {
		...
		applied, err := s.Repo.FailStaleUnsubmittedBatchImageJob(jobCtx, job.BatchID, cutoff, "SUBMIT_STALE_BEFORE_PROVIDER", msg)
		if err != nil {
		...
		if err := releaseBatchImageBalanceHold(jobCtx, s.Billing, job, batchImageDerefString(job.RequestHash)); err != nil {
			...
			s.enqueueReleaseRetry(jobCtx, job.BatchID)
			lastErr = err
			continue
		}
