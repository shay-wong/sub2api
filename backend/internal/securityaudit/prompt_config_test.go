...
func TestConfigManagerPublicRequiresSuccessfullyLoadedSnapshot(t *testing.T) {
	...
	t.Run("reload failure preserves last successfully loaded snapshot", func(t *testing.T) {
		...
		manager := NewConfigManager(nil, repository, nil, prefixEncryptor{}, true)
		require.NoError(t, manager.Reload(context.Background()))
...
// Runtime health must turn degraded only when credential recovery leaves no usable audit endpoint.
func TestPromptRuntimeDegradesOnlyWhenEveryEndpointTokenIsInvalid(t *testing.T) {
	tests := []struct {
		name              string
		blocking          bool
		includeValid      bool
		wantNoEndpointErr bool
	}{
		{name: "all endpoint tokens invalid in async mode", wantNoEndpointErr: true},
		{name: "all endpoint tokens invalid in blocking mode", blocking: true, wantNoEndpointErr: true},
		{name: "one endpoint token remains valid", includeValid: true},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			storage.BlockingEnabled = tc.blocking
			storage.ConfigVersion = 9
			storage.Endpoints = []StorageEndpoint{{
				ID: "g1", Name: "Guard 1", Protocol: "openai_compatible", BaseURL: "http://127.0.0.1:8080",
				Model: "m", TokenCiphertext: "undecryptable", TimeoutMS: 1000, InputLimit: 1000, Enabled: true,
			}}
			if tc.includeValid {
				storage.Endpoints = append(storage.Endpoints, StorageEndpoint{
					ID: "g2", Name: "Guard 2", Protocol: "openai_compatible", BaseURL: "http://127.0.0.1:8081",
					Model: "m", TokenCiphertext: "enc:usable", TimeoutMS: 1000, InputLimit: 1000, Enabled: true,
				})
			}
			persisted, err := json.Marshal(storage)
			require.NoError(t, err)

			manager := NewConfigManager(nil, staticSettingRepository{values: map[string]string{
				SettingKeyPromptAuditConfig: string(persisted),
				SettingKeyRiskControl:       "true",
			}}, nil, prefixEncryptor{}, true)
			require.NoError(t, manager.Reload(context.Background()))

			service := NewPromptService(manager, nil, nil, NewOpenAICompatibleScanner(), NewAtomicMetrics())
			runtime := service.Runtime(context.Background())
			require.Equal(t, tc.wantNoEndpointErr, runtime.LastErrorCode == ErrorCodeNoEnabledEndpoint)
			if tc.wantNoEndpointErr {
				require.Equal(t, "degraded", runtime.ProcessStatus)
				require.Equal(t, stableErrorMessage(ErrorCodeNoEnabledEndpoint), runtime.LastErrorMessage)
			}
		})
	}
}
...
func TestConfigManagerStartupLoadFailureDoesNotBlockWhenBlockingNotIntended(t *testing.T) {
	...
	manager := NewConfigManager(nil, errorSettingRepository{}, nil, prefixEncryptor{}, true)
