...
func TestPromptAuditConfigCASSecretRoundTripInvalidationAndTTL(t *testing.T) {
	...
	managerOne := NewConfigManager(db, settingRepo, redisClient, encryptor, true)
	managerTwo := NewConfigManager(db, settingRepo, redisClient, encryptor, true)
	...
	ttlManager := NewConfigManager(db, settingRepo, nil, encryptor, true)
	...
	degraded := NewConfigManager(db, settingRepo, deadRedis, encryptor, true)
