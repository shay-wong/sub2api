...
func TestDeriveOpenAIForwardAttemptBody_CrossModeStripsKiroReasoning(t *testing.T) {
	...
	state := &openAIPassthroughFailoverState{historyMayContainPassthrough: true, historyKnown: true}
...
func TestDeriveOpenAIForwardAttemptBody_SameModePreservesReasoning(t *testing.T) {
	...
	t.Run("non_passthrough_to_non_passthrough", func(t *testing.T) {
		state := &openAIPassthroughFailoverState{historyKnown: true}
...
// Missing provenance is fail-safe during rollout: a first non-passthrough
// selection strips encrypted history instead of waiting for a prior attempt.
func TestDeriveOpenAIForwardAttemptBody_UnknownSourceSanitizesFirstNonPassthrough(t *testing.T) {
	h := &OpenAIGatewayHandler{}
	canonical := []byte(kiroReasoningCanonicalBody)
	state := &openAIPassthroughFailoverState{}

	got := h.deriveOpenAIForwardAttemptBody(
		nil,
		canonical,
	...
	require.Equal(t, 0, reasoningItemCount(t, got))
	require.True(t, state.historyKnown)
	require.True(t, state.historyMayContainPassthrough)
	require.JSONEq(t, kiroReasoningCanonicalBody, string(canonical))
}

// Once unknown history reveals provider-specific encrypted reasoning, later
// turns stay conservative because clients resend the full conversation.
func TestDeriveOpenAIForwardAttemptBody_UnknownSourceStaysSanitizedAcrossTurns(t *testing.T) {
	h := &OpenAIGatewayHandler{}
	canonical := []byte(kiroReasoningCanonicalBody)
	state := &openAIPassthroughFailoverState{}
	account := newOpenAIPassthroughAccount(61, false)

	first := h.deriveOpenAIForwardAttemptBody(nil, canonical, account, state)
	second := h.deriveOpenAIForwardAttemptBody(nil, canonical, account, state)

	require.Equal(t, 0, reasoningItemCount(t, first))
	require.Equal(t, 0, reasoningItemCount(t, second))
	require.True(t, state.historyKnown)
	require.True(t, state.historyMayContainPassthrough)
}

func TestDeriveOpenAIForwardAttemptBody_ForceChatOverridesPassthroughSetting(t *testing.T) {
	h := &OpenAIGatewayHandler{}
	canonical := []byte(kiroReasoningCanonicalBody)
	account := newOpenAIPassthroughAccount(70, true)
	...
	got := h.deriveOpenAIForwardAttemptBody(
		...
		canonical,
		account,
		&openAIPassthroughFailoverState{historyMayContainPassthrough: true, historyKnown: true},
	)
