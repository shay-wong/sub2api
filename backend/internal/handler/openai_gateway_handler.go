...
func (h *OpenAIGatewayHandler) resolveResponsesWebSocketCompositeRoute(
	ctx context.Context,
	...
	if err != nil || !decision.Matched {
		return ctx, model, false, err
	}
	upstreamModel := strings.TrimSpace(decision.UpstreamModel)
	if upstreamModel == "" {
		upstreamModel = model
	}
	return service.WithCompositeRouteDecision(ctx, decision), upstreamModel, true, nil
}
...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	for {
		...
		h.recordCyberPolicyIfMarked(c, apiKey, selection, account, subscription, reqModel, err != nil, cyberBlockBodyHTTP, clientRequestedUsageFields(c, channelMapping, reqModel, ""), service.HashUsageRequestPayload(effectiveBody))
...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	for {
		...
		h.recordCyberPolicyIfMarked(c, apiKey, selection, account, subscription, reqModel, err != nil, cyberBlockBodyMsg, clientRequestedUsageFields(c, channelMappingMsg, reqModel, ""), service.HashUsageRequestPayload(body))
		forwardDurationMs := time.Since(forwardStart).Milliseconds()
		upstreamLatencyMs, _ := getContextInt64(c, service.OpsUpstreamLatencyMsKey)
		responseLatencyMs := forwardDurationMs
		if upstreamLatencyMs > 0 && forwardDurationMs > upstreamLatencyMs {
...
func (h *OpenAIGatewayHandler) acquireOpenAIAccountSlot(
	...
	if selection.Acquired {
		...
		return wrapAccountReleaseOnForwardDone(ctx, reqStream, selection.ReleaseFunc), openAISlotAcquireOK
	}
	...
	if fastAcquired {
		...
		return wrapAccountReleaseOnForwardDone(ctx, reqStream, fastReleaseFunc), openAISlotAcquireOK
	}
	...
	return wrapAccountReleaseOnForwardDone(ctx, reqStream, accountReleaseFunc), openAISlotAcquireOK
}
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	publicReqModel := strings.TrimSpace(gjson.GetBytes(firstMessage, "model").String())
	if publicReqModel == "" {
	...
	reqLog = reqLog.With(
		...
		zap.String("model", publicReqModel),
		zap.String("upstream_model", reqModel),
		zap.String("session_initial_model", publicReqModel),
		zap.Bool("has_previous_response_id", previousResponseID != ""),
		zap.String("previous_response_id_kind", previousResponseIDKind),
	)
	setOpsRequestContext(c, publicReqModel, true)
	setOpsEndpointContext(c, "", int16(service.RequestTypeWSV2))

	if decision := h.checkSecurityAuditStage(c, reqLog, apiKey, subject, service.ContentModerationProtocolOpenAIResponses, publicReqModel, firstMessage, "first_turn"); decision != nil && !decision.AllowNextStage {
	...
	for {
		...
		maxReasoningEffort, reasoningEffortMappings := EffectiveOpenAIReasoningEffortPolicy(selection, apiKey)
...
func cyberSessionBodyWithFallback(body []byte, fallbackSessionID string) []byte {
	fallbackSessionID = strings.TrimSpace(fallbackSessionID)
	if fallbackSessionID == "" || strings.TrimSpace(gjson.GetBytes(body, "prompt_cache_key").String()) != "" {
		return body
	}
	withFallback, err := sjson.SetBytes(body, "prompt_cache_key", fallbackSessionID)
	if err != nil {
		return body
	}
	return withFallback
