...
type passkeyIdentityRequest struct {
	EmailCode string `json:"email_code"`
	Password  string `json:"password"`
}

func bindPasskeyIdentity(c *gin.Context) passkeyIdentityRequest {
	var req passkeyIdentityRequest
...
func (h *PasskeyHandler) BeginRegistration(c *gin.Context) {
	...
	verification := bindPasskeyIdentity(c)
	creation, token, err := h.passkeys.BeginRegistration(
		c.Request.Context(),
		subject.UserID,
...
func (h *PasskeyHandler) GetVerificationMethod(c *gin.Context) {
	if !h.requirePasskeysEnabled(c) {
		return
	}
	subject, ok := middleware2.GetAuthSubjectFromContext(c)
	...
	method, err := h.passkeys.GetVerificationMethod(c.Request.Context(), subject.UserID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *PasskeyHandler) SendVerifyCode(c *gin.Context) {
	if !h.requirePasskeysEnabled(c) {
		return
	}
	subject, ok := middleware2.GetAuthSubjectFromContext(c)
	...
	if err := h.passkeys.SendVerifyCode(
		c.Request.Context(),
		subject.UserID,
