...
func setupRestrictedProxyDataRouter(scope service.AdminResourceScope) (*gin.Engine, *stubAdminService) {
	gin.SetMode(gin.TestMode)
	router := gin.New()
	adminSvc := newStubAdminService()
	...
	h := NewProxyHandler(adminSvc, permissionSvc)
...
func TestRestrictedAdminProxyImportDoesNotReuseUnboundProxy(t *testing.T) {
	router, adminSvc := setupRestrictedProxyDataRouter(service.AdminResourceScope{
		Mode:     service.AdminResourceScopeRestricted,
		ProxyIDs: []int64{2},
	})
	adminSvc.proxies = []service.Proxy{
		{ID: 1, Name: "unbound", Protocol: "http", Host: "127.0.0.1", Port: 8080, Status: service.StatusActive},
		{ID: 2, Name: "bound", Protocol: "https", Host: "127.0.0.2", Port: 8443, Status: service.StatusActive},
	}
	payload := map[string]any{"data": map[string]any{
		"type": dataType, "version": dataVersion,
		"proxies": []map[string]any{{
			"proxy_key": "http|127.0.0.1|8080||", "name": "unbound", "protocol": "http", "host": "127.0.0.1", "port": 8080, "status": "inactive",
		}},
		"accounts": []map[string]any{},
	}}
	body, _ := json.Marshal(payload)
	...
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &resp))
	require.Equal(t, 1, resp.Data.ProxyCreated)
	require.Zero(t, resp.Data.ProxyReused)
	require.NotContains(t, adminSvc.updatedProxyIDs, int64(1))
}
