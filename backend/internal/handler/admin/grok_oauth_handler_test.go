...
func newProjectGrokOAuthScopeRouter(adminSvc *stubAdminService) *gin.Engine {
	gin.SetMode(gin.TestMode)
	router := gin.New()
	handler := NewGrokOAuthHandler(service.NewGrokOAuthService(nil, nil), adminSvc, nil, nil, nil)
...
func TestProjectGrokOAuthRoutesAllowProjectScopedProxyUse(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:       "validate_sso_token",
			path:       "/api/v1/admin/grok/oauth/sso-token",
			body:       `{"sso_token":"token","proxy_id":4}`,
			wantStatus: http.StatusInternalServerError,
			wantReason: "GROK_OAUTH_CLIENT_NOT_CONFIGURED",
		},
		{
			name:       "authorize_password",
			path:       "/api/v1/admin/grok/oauth/password",
			body:       `{"email":"user@example.com","password":"password","proxy_id":4}`,
			wantStatus: http.StatusForbidden,
			wantReason: "GROK_OAUTH_PASSWORD_AUTH_DISABLED",
		},
	}
