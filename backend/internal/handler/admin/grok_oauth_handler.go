...
func (h *GrokOAuthHandler) CreateAccountsFromSSO(c *gin.Context) {
	...
	for i := 0; i < workerCount; i++ {
		...
		go func() {
			defer wg.Done()
			for job := range jobs {
				items[job.index] = h.safeCreateAccountFromSSOToken(ctx, req, job.token, job.index+1, len(tokens), scope)
			}
		}()
	}
...
func (h *GrokOAuthHandler) safeCreateAccountFromSSOToken(ctx context.Context, req GrokSSOToOAuthRequest, token string, index, total int, scopes ...*adminAccessScope) (result grokSSOImportWorkerResult) {
	defer func() {
	...
	return h.createAccountFromSSOToken(ctx, req, token, index, total, scopes...)
}

func (h *GrokOAuthHandler) createAccountFromSSOToken(ctx context.Context, req GrokSSOToOAuthRequest, token string, index, total int, scopes ...*adminAccessScope) grokSSOImportWorkerResult {
	...
	if len(scopes) > 0 && scopes[0] != nil {
		responseAccount = scopes[0].accountForResponse(account)
	}
