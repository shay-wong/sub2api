...
// mergeOmittedValidationFields builds the effective settings document used by
// cross-field validation. Persistence still receives omitted separately and
// drops those keys, so filling the validation copy cannot overwrite stored
// values the caller did not submit.
func mergeOmittedValidationFields(req *UpdateSettingsRequest, previous *service.SystemSettings, omitted service.OmittedSettingKeys) {
	requestValue := reflect.ValueOf(req).Elem()
	requestType := requestValue.Type()
	previousValue := reflect.ValueOf(previous).Elem()

	for i := 0; i < requestType.NumField(); i++ {
		requestFieldType := requestType.Field(i)
		if requestFieldType.Type.Kind() == reflect.Pointer {
			continue
		}
		jsonName, _, _ := strings.Cut(requestFieldType.Tag.Get("json"), ",")
		settingKey, ok := settingKeyByJSONName[jsonName]
		if !ok || !omitted.IsOmitted(settingKey) {
			continue
		}
		previousField := previousValue.FieldByName(requestFieldType.Name)
		requestField := requestValue.Field(i)
		if previousField.IsValid() && previousField.Type().AssignableTo(requestField.Type()) {
			requestField.Set(previousField)
		}
	}
}
...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	mergeOmittedValidationFields(&req, previousSettings, omitted)
