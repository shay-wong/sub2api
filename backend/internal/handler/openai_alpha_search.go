...
func (h *OpenAIGatewayHandler) AlphaSearch(c *gin.Context) {
	...
	// Channel mapping belongs to the API key's requested group. A fallback
	// selection group changes billing and rate-limit attribution, not model intent.
	...
	for {
		...
		accountRelease, acquired := h.acquireResponsesAccountSlot(c, selectedGroupID, sessionHash, selection, false, &streamStarted, reqLog)
		if !acquired {
			return
		}
		...
		h.recordCyberPolicyIfMarked(
			...
			err != nil,
			cyberBlockKey,
			channelMapping.ToUsageFields(requestedModel, ""),
		...
		if err == nil {
			...
			if result != nil {
				h.recordAlphaSearchUsage(c, selection, apiKey, account, subscription, channelMapping, requestedModel, body, result, subject.UserID)
			}
			return
		}
		...
		h.gatewayService.ReportOpenAIAccountScheduleFailover(account.ID, failoverErr)
		if c.Writer.Size() != writerSizeBeforeForward {
