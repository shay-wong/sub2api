...
func RegisterAdminRoutes(
	...
	admin.Use(middleware.AdminComplianceGuard(settingService))
	{
		...
		// 数据管理
		registerDataManagementRoutes(adminOnly, h, stepUpAuth)

		// 数据库备份恢复
		registerBackupRoutes(adminOnly, h, stepUpAuth)
		...
		// 操作审计日志
		registerAuditLogRoutes(adminOnly, h, stepUpAuth)
	}
}
...
func registerOpsRoutes(admin *gin.RouterGroup, h *handler.Handlers, panelRateLimiter *middleware.PanelRateLimiter) {
	ops := admin.Group("/ops")
	{
		...
		ops.GET("/concurrency", opsRead, h.Admin.Ops.GetConcurrencyStats)
		ops.GET("/user-concurrency", adminOnly, h.Admin.Ops.GetUserConcurrencyStats)
		ops.GET("/account-availability", opsRead, h.Admin.Ops.GetAccountAvailability)
		ops.GET("/realtime-traffic", opsRead, h.Admin.Ops.GetRealtimeTrafficSummary)

		// Alerts (rules + events)
		ops.GET("/alert-rules", adminOnly, h.Admin.Ops.ListAlertRules)
		ops.POST("/alert-rules", adminOnly, h.Admin.Ops.CreateAlertRule)
		ops.PUT("/alert-rules/:id", adminOnly, h.Admin.Ops.UpdateAlertRule)
		ops.DELETE("/alert-rules/:id", adminOnly, h.Admin.Ops.DeleteAlertRule)
		ops.GET("/alert-events", opsRead, h.Admin.Ops.ListAlertEvents)
		ops.GET("/alert-events/:id", opsRead, h.Admin.Ops.GetAlertEvent)
		ops.PUT("/alert-events/:id/status", adminOnly, h.Admin.Ops.UpdateAlertEventStatus)
		ops.POST("/alert-silences", adminOnly, h.Admin.Ops.CreateAlertSilence)
		...
		runtime := ops.Group("/runtime")
		{
			...
			runtime.PUT("/logging", adminOnly, h.Admin.Ops.UpdateRuntimeLogConfig)
			runtime.POST("/logging/reset", adminOnly, h.Admin.Ops.ResetRuntimeLogConfig)
		}

		// Advanced settings (DB-backed)
		ops.GET("/advanced-settings", adminOnly, h.Admin.Ops.GetAdvancedSettings)
		ops.PUT("/advanced-settings", adminOnly, h.Admin.Ops.UpdateAdvancedSettings)

		// Settings group (DB-backed)
		settings := ops.Group("/settings")
		{
			settings.GET("/metric-thresholds", adminOnly, h.Admin.Ops.GetMetricThresholds)
			settings.PUT("/metric-thresholds", adminOnly, h.Admin.Ops.UpdateMetricThresholds)
		}

		// WebSocket realtime (QPS/TPS)
		ws := ops.Group("/ws")
		{
			ws.GET("/qps", opsRead, h.Admin.Ops.QPSWSHandler)
		}

		// Error logs (legacy)
		ops.GET("/errors", opsRead, h.Admin.Ops.GetErrorLogs)
		ops.GET("/errors/:id", opsRead, h.Admin.Ops.GetErrorLogByID)
		ops.PUT("/errors/:id/resolve", adminOnly, h.Admin.Ops.UpdateErrorResolution)

		// Request errors (client-visible failures)
		ops.GET("/request-errors", opsRead, h.Admin.Ops.ListRequestErrors)
		ops.GET("/request-errors/:id", opsRead, h.Admin.Ops.GetRequestError)
		ops.GET("/request-errors/:id/upstream-errors", opsRead, h.Admin.Ops.ListRequestErrorUpstreamErrors)
		ops.PUT("/request-errors/:id/resolve", adminOnly, h.Admin.Ops.ResolveRequestError)
		...
		ops.GET("/upstream-errors", opsRead, h.Admin.Ops.ListUpstreamErrors)
		ops.GET("/upstream-errors/:id", opsRead, h.Admin.Ops.GetUpstreamError)
		ops.PUT("/upstream-errors/:id/resolve", adminOnly, h.Admin.Ops.ResolveUpstreamError)

		// Request drilldown (success + error)
		ops.GET("/requests", opsRead, h.Admin.Ops.ListRequestDetails)

		// Indexed system logs
		ops.GET("/system-logs", adminOnly, h.Admin.Ops.ListSystemLogs)
		ops.POST("/system-logs/cleanup", adminOnly, h.Admin.Ops.CleanupSystemLogs)
...
func registerDashboardRoutes(admin *gin.RouterGroup, h *handler.Handlers, panelRateLimiter *middleware.PanelRateLimiter) {
	...
	dashboard.Use(panelRateLimiter.Heavy())
	{
		...
		dashboard.GET("/snapshot-v2", dashboardRead, h.Admin.Dashboard.GetSnapshotV2)
		dashboard.GET("/stats", dashboardRead, h.Admin.Dashboard.GetStats)
		dashboard.GET("/realtime", dashboardRead, h.Admin.Dashboard.GetRealtimeMetrics)
...
func registerUserManagementRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	users.Use(middleware.RequireAdminPermission(service.AdminPermissionUsersManage))
	{
		...
		users.POST("/batch-limits", middleware.RequireAdminOnly(), h.Admin.User.BatchUpdateLimits)
...
func registerGroupRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	{
		groupManage := middleware.RequireAdminPermission(service.AdminPermissionGroupsManage)

		groups.GET("", groupManage, h.Admin.Group.List)
		...
		scopedGroup.PUT("/composite-routes/:route_id", h.Admin.Group.UpdateCompositeRoute)
		scopedGroup.DELETE("/composite-routes/:route_id", h.Admin.Group.DeleteCompositeRoute)
		scopedGroup.GET("", h.Admin.Group.GetByID)
		scopedGroup.POST("/duplicate", h.Admin.Group.Duplicate)
		scopedGroup.PUT("", h.Admin.Group.Update)
		scopedGroup.DELETE("", h.Admin.Group.Delete)
		scopedGroup.GET("/stats", h.Admin.Group.GetStats)
		scopedGroup.GET("/rate-multipliers", h.Admin.Group.GetGroupRateMultipliers)
		scopedGroup.PUT("/rate-multipliers", h.Admin.Group.BatchSetGroupRateMultipliers)
		scopedGroup.DELETE("/rate-multipliers", h.Admin.Group.ClearGroupRateMultipliers)
		scopedGroup.PUT("/rpm-overrides", h.Admin.Group.BatchSetGroupRPMOverrides)
		scopedGroup.DELETE("/rpm-overrides", h.Admin.Group.ClearGroupRPMOverrides)
		scopedGroup.GET("/api-keys", h.Admin.Group.GetGroupAPIKeys)
	}
}
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	...
	accounts.Use(middleware.RequireAdminPermission(service.AdminPermissionAccountsWrite))
	{
		...
		accounts.GET("/upstream-billing-probe/settings", adminOnly, h.Admin.Account.GetUpstreamBillingProbeSettings)
		accounts.PUT("/upstream-billing-probe/settings", adminOnly, h.Admin.Account.UpdateUpstreamBillingProbeSettings)
		accounts.POST("/upstream-billing-probe/batch", adminOnly, h.Admin.Account.ProbeUpstreamBillingBatch)
		accounts.GET("/ollama-cloud-usage/settings", adminOnly, h.Admin.Account.GetOllamaCloudUsageSettings)
		accounts.PUT("/ollama-cloud-usage/settings", adminOnly, h.Admin.Account.UpdateOllamaCloudUsageSettings)
		...
		accounts.PUT("/:id/ollama-cloud-usage/session", adminOnly, h.Admin.Account.SaveOllamaCloudUsageSession)
		accounts.DELETE("/:id/ollama-cloud-usage/session", adminOnly, h.Admin.Account.DeleteOllamaCloudUsageSession)
		accounts.PUT("/:id/ollama-cloud-usage/auto-refresh", adminOnly, h.Admin.Account.SetOllamaCloudUsageAutoRefresh)
		accounts.POST("/:id/ollama-cloud-usage/refresh", adminOnly, h.Admin.Account.RefreshOllamaCloudUsage)
		...
		accounts.GET("/data", adminOnly, gin.HandlerFunc(stepUpAuth), h.Admin.Account.ExportData)
		accounts.POST("/data", adminOnly, h.Admin.Account.ImportData)
...
func registerGrokOAuthRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	grok := admin.Group("/grok")
	{
		...
		grok.POST("/accounts/:id/refresh", accountWrite, h.Admin.Account.RefreshGrok)
...
func registerProxyRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	...
	proxies.Use(middleware.RequireAdminPermission(service.AdminPermissionProxiesManage))
	{
		...
		proxies.GET("/data", adminOnly, gin.HandlerFunc(stepUpAuth), h.Admin.Proxy.ExportData)
		proxies.POST("/data", h.Admin.Proxy.ImportData)
		proxies.POST("", h.Admin.Proxy.Create)
		proxies.POST("/batch-delete", h.Admin.Proxy.BatchDelete)
		proxies.POST("/batch", h.Admin.Proxy.BatchCreate)

		scopedProxy := proxies.Group("/:id", h.Admin.Proxy.RequireResourceScope())
		scopedProxy.GET("", h.Admin.Proxy.GetByID)
		scopedProxy.PUT("", h.Admin.Proxy.Update)
		scopedProxy.DELETE("", h.Admin.Proxy.Delete)
		scopedProxy.POST("/test", h.Admin.Proxy.Test)
		scopedProxy.POST("/quality-check", h.Admin.Proxy.CheckQuality)
		scopedProxy.GET("/stats", h.Admin.Proxy.GetStats)
		scopedProxy.GET("/accounts", h.Admin.Proxy.GetProxyAccounts)
	}
}
...
func registerSubscriptionRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	subscriptionManage := middleware.RequireAdminPermission(service.AdminPermissionSubsManage)
	subscriptions.Use(subscriptionManage)
	{
		...
		scopedSubscription.GET("/progress", h.Admin.Subscription.GetProgress)
		scopedSubscription.POST("/extend", h.Admin.Subscription.Extend)
		scopedSubscription.POST("/reset-quota", h.Admin.Subscription.ResetQuota)
		scopedSubscription.POST("/revoke", h.Admin.Subscription.Revoke)
		scopedSubscription.POST("/restore", h.Admin.Subscription.Restore)
		scopedSubscription.DELETE("", h.Admin.Subscription.Revoke)
	}

	// 分组下的订阅列表
	admin.GET("/groups/:id/subscriptions", subscriptionManage, h.Admin.Subscription.ListByGroup)
...
func registerUsageRoutes(admin *gin.RouterGroup, h *handler.Handlers, panelRateLimiter *middleware.PanelRateLimiter) {
	...
	adminOnly := middleware.RequireAdminOnly()
	{
		usage.GET("", usageRead, h.Admin.Usage.List)
		usage.GET("/count", usageRead, h.Admin.Usage.Count)
		usage.GET("/stats", usageRead, h.Admin.Usage.Stats)
		usage.GET("/search-users", usageRead, h.Admin.Usage.SearchUsers)
		usage.GET("/search-api-keys", usageRead, h.Admin.Usage.SearchAPIKeys)
		usage.GET("/search-models", usageRead, h.Admin.Usage.SearchModels)
		usage.GET("/search-accounts", usageRead, h.Admin.Usage.SearchAccounts)
		usage.GET("/search-groups", usageRead, h.Admin.Usage.SearchGroups)
