...
| 上游默认分支 | `main` |
| 已合并上游提交 / 比较基线 | `b1748c4ea99ce2120401a269142aa071e18a84da` |
| 当前比较范围 | `b1748c4ea99ce2120401a269142aa071e18a84da..HEAD` |

`upstream/main` 是移动目标，不自动等于本文档基线。本次合并固定上游提交为 `b1748c4ea99ce2120401a269142aa071e18a84da`；远端后续推进不改变本次 merge 的第二父，下一次审计仍须从最新已合并上游 merge 重新确定基线。
...
- **行为不变量**：真实 fallback 后的检查、利润准入、倍率、订阅计费、免费 Fast、reasoning effort deny/downgrade、账号统计和用量日志必须使用 `AccountSelectionResult` 的有效分组，不得回退到原 `apiKey.Group`；composite 只路由目标平台、不携带成员分组关系，因此 selection 与 usage 继续归属于 API Key 所属父分组，且 composite 本身不得凭空安装成员分组利润门；同一请求共享一个 `PricingAt`。API Key 自身的 5 小时限额继续独立生效，不属于本能力。
...
- **迁移与测试**：`backend/migrations/145_group_5h_rate_limits.sql` 仅保留为已执行历史；`backend/migrations/235_drop_group_5h_rate_limits.sql` 删除分组 5 小时字段和窗口表，`backend/migrations/group_5h_rate_limit_removal_migration_test.go` 锁定删除顺序并确认 API Key 5 小时限额仍存在。鉴权缓存快照使用 `v23`，防止字段含义不同的旧 `v22` 快照绕过上游 Fast/reasoning policy 字段刷新。实际分组归因由 `backend/internal/handler/endpoint_test.go`、`backend/internal/handler/openai_reasoning_policy_contract_test.go`、`backend/internal/service/api_key_auth_cache_openai_fast_test.go`、`backend/internal/service/api_key_auth_cache_profit_test.go`、`backend/internal/service/gateway_record_usage_test.go`、`backend/internal/service/openai_gateway_record_usage_test.go`、`backend/internal/service/gateway_profit_control_v2_test.go`、`backend/internal/service/openai_profit_control_paths_test.go` 和 `backend/internal/service/response_model_billing_test.go` 覆盖。
...
- **合并审查**：重点检查 gateway 预检、账户切换、sticky binding、reasoning policy、免费 Fast 重算、auth snapshot、usage worker 和图像/Grok 分支；同名 `group_id` 不代表已使用实际调度分组。
...
- **Ultrafast 协议不变量**：Responses、Chat Completions 与 Responses WebSocket 必须把官方 `service_tier: "ultrafast"` 原样转发，不能转换为 `priority`，也不在 Sub2API 硬编码模型白名单；Fast Policy 可单独匹配 Ultrafast，`force_priority` 仍明确表示主动降为普通 Fast；OpenAI OAuth 的私有 `x-codex-routing-hint` 必须按最终出站模型与档位生成 `model=<model>;tier=ultrafast`，不能只保留模型提示。
...
- **GPT-6 Astra 不变量**：`gpt-6-astra` 必须原样转发，并在默认模型、前端白名单和 Codex 模型目录中可见；Codex 元数据声明图片输入、922K 最大输入、`low/medium/high/xhigh/max` 推理档位和 Priority/Fast。远程与内置离线目录按官方标准价（输入 $10/M、缓存读取 $1/M、缓存写入 $12.5/M、输出 $50/M）及 Priority `2x` 计费，超过 272K 输入时整次请求输入 `2x`、输出 `1.5x`；静态故障兜底只包含基础价，不推测长上下文阶梯。账号测试默认模型继续使用 `gpt-5.6-sol`，避免新模型权限尚未普及时改变健康检查行为。
- **GPT-6 Astra 实现与文档**：代码位于 `backend/internal/pkg/openai/constants.go`、`backend/internal/service/openai_model_alias.go`、`backend/internal/service/openai_codex_transform.go`、`backend/internal/service/openai_gateway_request_body.go`、`backend/internal/service/openai_codex_models_service.go`、`backend/internal/service/pricing_service.go`、`backend/internal/service/billing_service.go`、`backend/resources/model-pricing/model_prices_and_context_window.json`、`frontend/src/composables/useModelWhitelist.ts`；测试位于对应 Go 测试文件和 `frontend/src/composables/__tests__/useModelWhitelist.spec.ts`；用户文档为 `README.md`、`README_CN.md`、`README_JA.md`。当前上游基线 `b1748c4ea99ce2120401a269142aa071e18a84da` 尚无 GPT-6 Astra 专门适配。
...
- **本次基线继续吸收**：`530fb20f2` 将长上下文阶梯价迁入价格目录，`593fc9365` 提供目录 override file，`e2cfaa46e` 修正目录缓存字段契约和 Gemini Pro 缓存写入价，`9eabd2a5b` 让账号统计应用相同模型计价策略。上游统一计价路径取代 fork 的旁路 helper，但不改变 Ultrafast 默认倍率/渠道覆盖、canonical CNY rate、公开 DTO、provider snapshot 与退款状态机边界。
...
- **本次基线新增吸收**：`e9e3c46cb`、`4c1f920d5` 与 `abc07bb07` 将实际代理与上游端点归因覆盖到全部转发和失败路径，并对异步 Ops 事件做有界快照；该能力来自上游，不登记为 fork 差异，也不替代 fork 的共享筛选、deleted-key digest 与隐私约束。
...
- **本次基线新增吸收**：`863667ce6` 与 `b6a7b8b7d` 为数据库启动和 Ent 初始化增加瞬时错误重试，部署示例同时补充价格目录覆盖挂载。这些是上游原生运维能力，不登记为 fork 差异，也不替代相对 URL、示例 YAML 可解析和 migration `177` 恢复契约。
