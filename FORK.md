...
| 上游默认分支 | `main` |
| 已合并上游提交 / 比较基线 | `1b5dc676a9d35532ac2d88dbbe0ee2638b2ab05f` |
| 当前比较范围 | `1b5dc676a9d35532ac2d88dbbe0ee2638b2ab05f..HEAD` |

`upstream/main` 是移动目标，不自动等于本文档基线。本次合并固定上游提交为 `1b5dc676a9d35532ac2d88dbbe0ee2638b2ab05f`；远端后续推进不改变本次 merge 的第二父，下一次审计仍须从最新已合并上游 merge 重新确定基线。
...
| 已发布同基线 Fork 版本 | `0.1.178-fork.1` |
| 下次发布所需版本 | `0.1.178-fork.2` |
...
- **待合并上游审查**：`upstream/main` 在 `1b5dc676a9d35532ac2d88dbbe0ee2638b2ab05f` 仍未覆盖 subscription 日、周、月限额的 429 映射。
...
- **本次合并解决**：合入上游父提交 `1b5dc676a9d35532ac2d88dbbe0ee2638b2ab05f` 时，Responses WS composite 路由继续按 Project/模型决定目标平台；WS HTTP bridge 在 fork 的 Grok client-tool 恢复循环内接入上游首输出暂存与后续 turn 429 failover；测试桩同步新增 reasoning-content cache 接口。
...
- **当前基线上游原生能力**：Composite 图片/Codex/CN 路由、daily-midnight subscription reset、Channel Monitor V2、response-model/service-tier 与渠道区间倍率计费、系统日志退避、Grok 4.6/JWT tier/x_search/inline-image、Codex OAuth 指纹 opt-in、分组每日 rollup、remote compaction v2/turn-state provenance、request-scoped capacity recovery、Responses WS 后续 turn 429 failover、client-tool follow-up、用量单次聚合、Fast/Flex 管理文案和 Go 1.26.6 builder 对齐均来自 `1b5dc676a9d35532ac2d88dbbe0ee2638b2ab05f` 基线，不登记为 fork 能力，也不得在后续冲突中因同名本地 helper 而删除。
