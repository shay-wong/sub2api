...
> - **国际渠道 / 美元结算** — [Kyren Pay](https://merchant.kyrenpay.com/?code=SUB2API)（`https://merchant.kyrenpay.com/?code=SUB2API`）：支持国际版微信支付与支付宝、本地货币支付和美元结算。当前费率、提现方式、结算门槛及账户资格以 Kyren Pay 最新条款和审核状态为准；开通结算前可能需要完成 KYC 或 KYB。详见 Kyren Pay 的[资金管理](https://docs.kyrenpay.com/dashboard/funds)、[KYC](https://docs.kyrenpay.com/dashboard/kyc) 和 [KYB](https://docs.kyrenpay.com/dashboard/kyb) 文档。注册链接包含 Sub2API 邀请码；如不希望使用，可移除 `code` 参数。
>
