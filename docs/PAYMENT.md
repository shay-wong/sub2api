...
> - **International channel / USD settlement** — [Kyren Pay](https://merchant.kyrenpay.com/?code=SUB2API) (`https://merchant.kyrenpay.com/?code=SUB2API`): supports WeChat Pay and Alipay with local-currency checkout and USD settlement. Current fees, withdrawal methods, settlement thresholds, and account eligibility depend on Kyren Pay's terms and review status; KYC or KYB may be required before settlement access. See Kyren Pay's [Funds](https://docs.kyrenpay.com/dashboard/funds), [KYC](https://docs.kyrenpay.com/dashboard/kyc), and [KYB](https://docs.kyrenpay.com/dashboard/kyb) documentation. The signup link contains Sub2API's referral code; remove the `code` parameter if you prefer not to use it.
>
