...
resolution for API Key access control and request metadata. Session binding is
the deliberate exception: it always uses Gin's `server.trusted_proxies` chain,
so a changing Docker or reverse-proxy peer is not mistaken for a changing
browser address. Custom headers from
