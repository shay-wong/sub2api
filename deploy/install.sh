...
# Sub2API 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/shay-wong/sub2api/stable/deploy/install.sh | bash
#
...
get_latest_version() {
    ...
    LATEST_VERSION=$(github_api_curl -s --connect-timeout 10 --max-time 30 "https://api.github.com/repos/${GITHUB_REPO}/releases?per_page=20" 2>/dev/null | grep '"tag_name"' | head -1 | sed -E 's/.*"([^"]+)".*/\1/')
...
get_current_version() {
    if [ -f "$INSTALL_DIR/sub2api" ]; then
        ...
        "$INSTALL_DIR/sub2api" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+(-fork\.[0-9]+)?' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
Documentation=https://github.com/shay-wong/sub2api
...
upgrade() {
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/sub2api" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+(-fork\.[0-9]+)?' || echo "unknown")
